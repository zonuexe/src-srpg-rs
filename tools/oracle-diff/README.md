...
> 3. **（残・要設計判断）精神の与ダメージ倍率が Rust ハードコード / SRC は sp.txt データ駆動**。本 fixture の sp.txt: 熱血=`ダメージ増加Lv10`(×2.0,一致)・
>    魂=`ダメージ増加Lv15`(C#×2.5 / Rust ハードコード×3)・気合=`気力増加Lv1`(C# 与ダメ非増 / Rust ×1.2)。さらに C# は複数
>    `ダメージ増加` を **MaxDbl で非加算** (大きい方)・Rust は積で**加算**。⇒ 精神倍率の data 駆動化＋ MaxDbl 化は要設計判断（未着手）。
