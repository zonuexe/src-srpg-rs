...
>    `db::sp_effect_level`（active 精神の最大 Lv）で解決して `predict` が `1 + 0.1*Lv`（MaxDbl）を適用。熱血+魂=×2.5(210)・
>    気合=×1.0(気力増加で与ダメ非増)。
> 4. ✅ **防御側 被ダメージ低下/増加も sp.txt データ駆動・C# 完全 up/down-mod へ是正済**（`e4aaa82`）。`DamageSpiritLevels` 構造体＋`db::sp_effect_level`。
>    **不屈 を `min(1)`→×0.1 是正**（被ダメージ低下Lv9。例 1572→157）。鉄壁=Lv7.5(÷4) 同値。バリア（強度吸収シールド）は範囲外で ÷2 近似のまま。
>    防御側 鉄壁/不屈 ケースを含め **morale corpus 13/13 一致**。捨て身/攻撃力ＵＰ は名前ベースのまま（範囲外）。
