...
> 与ダメージ系のパイロット技能を実装。**潜在力開放** (高気力 130 以上で与ダメ ×1.25、`748...`→`潜在力開放`、
> predict のダメージ段、静的 features 参照) / **得意技・不得手** (技能データ `得意技=格射` の各文字が武器 class に
> 一致で ×1.2 / ×0.8)。さらに **行動不能の防御側を絶対命中に**: 麻痺/睡眠/凍結/石化/行動不能 のとき命中 100
