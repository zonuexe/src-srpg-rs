...
**ブランチ／コミット**: **`feat/necessary-skill-gate`**（**本セッション 42 コミット・未 push**）。`origin/master`=`88ad16f` から先行。
...
- **★ D スパロボ戦記の「進行不能」を解決**: in-repo fixture を zip→`verify-archive` ドライブ＋新設 `VERIFY_ENTRY=@2`/`VERIFY_AUTOSTART`/`VERIFY_VAR`
  で **ブラウザ/84MB 無しに triage→完走**。エンジンはキャラメイキング→敵候補生成→戦闘配置まで正しく機能し、原因は harness の
  entry-point/メニュー操作の再現漏れだった（**エンジン修正不要**）。詳細は §2。
...
- ✅ **D スパロボ戦記（進行不能）＝解決済（2026-06-16・§2）**: `VERIFY_ENTRY="@2" VERIFY_AUTOSTART=1` で root から駆動すると
  ...
  →攻撃が静かに不発）。✅ **キャラメイキングの drive 自動化を実装**（`e6a8c36`）し、**1 人目のパイロット作成・ロスター追加まで実証**。
  **残ブロッカー**（次セッション）: ① 2 人目以降のテキスト入力がエンジン側で詰む（全角カタカナ判定の状態持ち越し疑い、要 src-core 調査。
  キャラメイキングは全機が埋まるまでループするため 2 人とも要作成）② ロスター追加≠機体搭乗（搭乗は後段工程）。詳細は §2。
...
- → **1 人目のパイロット `パイロア＝テスト` を作成し部隊ロスターに追加できることを実証**（`【システム】…を部隊に加えた。`）。

⑤ **残る具体的ブロッカー（次セッション・優先度順）**:
1. **2 人目以降のテキスト入力がエンジン側で詰む**: 1 人目は通るのに、同形式の 2 人目のカタカナ名（例 `パイロイ`）が
   `全角カタカナだけで入力してください`（CMaking.eve `名前入力` の `IsNumeric`/`Replace(文字)` 検証, L295-318）で誤拒否される。
   `respond_dialog_text` の繰り返し入力時の**状態持ち越し疑い**（要 src-core 調査）。キャラメイキングは**全 `パイロット不在` 機が埋まるまでループし
   手動 exit が無い**ため、**2 人とも作れないと出撃に進めない**（＝これが最優先）。
2. **ロスター追加 ≠ 機体搭乗**: キャラメイキングはパイロットを**ロスターに追加するだけ**で、`パイロット不在` 機への搭乗は**後段の別工程**
   （出撃準備／乗せ替え）。drive 終了時も `ガンダム pilot=""` のまま。1.解決後、搭乗工程の drive も要追加。
3. その後ようやく出撃→戦闘。**combat 自体は健全**（敵は `人工知能(ザコ)` で combat_data=✓、武器 firable）なので、味方に combat_data が付けば
   `VERIFY_AUTOPLAY` で戦闘が走るはず（→ 勝敗まで通す）。
