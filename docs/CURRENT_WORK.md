...
**次の一手（headless 続行可能）**: smoke drive は Title→プロローグ→戦闘へ直行し**キャラメイキングのメニュー（Menu/Ask/Input）に一切入らない**。
キャラメイキングは `lib/CMaking.eve`（1608 行）で、(a) player がメニューから選ぶ流れ（`Help.eve` のキャラメイキング1/2）か (b) 強制 auto-run の
はず。次は **drive にキャラメイキングを完走させる**（Input に既定名・Menu に妥当選択を与える）か、**なぜ戦闘へ直行するか**（出撃準備の入口
ロジック）を `VERIFY_VAR` で追う。CMaking.eve が使う Info/関数/コマンドの未実装が見つかればそこを実装。
