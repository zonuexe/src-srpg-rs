...
- ✅ **D スパロボ戦記（進行不能）＝解決済（2026-06-16・§2）**: `VERIFY_ENTRY="@2" VERIFY_AUTOSTART=1` で root から駆動すると
  Title→難易度設定→機体選択→サブ機体選択を完走し、**味方 9 機 vs 敵 6 機＋ボスの戦闘まで到達**（ブラウザ/84MB 不要）。
  「敵が出撃しない/進行不能」は**完全に harness（verify-archive のドライブ）の制約**で、エンジンのキャラメイキング・敵候補生成・敵配置は
  すべて正しく機能していた（**エンジン修正不要**）。残（任意・engine 非ブロック）: smoke drive で勝敗まで通すには move/attack を打つ drive 拡張
  （深掘り）、正規の名前入力再現には独自 click UI 駆動。いずれも進行不能ではない。詳細は §2。
...
# → Title→難易度設定→機体選択→サブ機体選択 を完走 (units 0→9) → 敵候補="ガンセクト" 敵陣営="インスペクター"
#   → Battle (scene=MapView, on_map=9/9): ガンダム[Player]×9 vs ガンセクト[Enemy]×6 + 機械獣アブドラＵ６[Enemy]
```
...
**残（任意・engine 非ブロック）**: ① smoke drive は Battle で EndPhase を繰り返すだけ（手を打たない）＝戦闘の通し（勝敗まで）を
見るには move/attack を打つ drive 拡張が要る（穴ではなく深掘り）。② キャラメイキングの名前等は auto-break で既定/空のまま（ハッシュ生成名）。
正規の名前入力を再現したいなら独自 click UI 駆動（§旧 (a)）。いずれも進行不能ではない。
...
| # | 課題 | 状況 |
|---|------|------|
| 戦記-CM | **スパロボ戦記 進行不能＝キャラメイキング**（§2） | ✅✅✅ **解決済（2026-06-16）**。`VERIFY_ENTRY="@2" VERIFY_AUTOSTART=1` で root から駆動するとキャラメイキング→敵候補生成→戦闘配置まで完走（味方 9 vs 敵 6＋ボス）。「進行不能」は**完全に harness のドライブ制約**（entry-point に Main.eve を選ぶ・autostart 時のクリック座標ずれ）で、エンジン修正は不要だった。残（任意・非ブロック）: 勝敗まで通す move/attack drive 拡張、正規の名前入力の click UI 駆動 |
