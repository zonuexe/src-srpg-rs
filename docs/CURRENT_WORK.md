...
> **2026-06-15 追記 (続き12 — C 敵AI 防御地形選好)**: 残課題サマリ C「地形/陣形の戦術判断」を一部消化 (`0188d19`)。
> `ai_act_unit` の best_move (ターゲットを射程内に収める到達マスの選択) で、攻撃可能なマスのうち防御地形を選好するよう
> min_by_key を変更。新ヘルパ `tile_defensive_value(x,y)=terrain_damage_mod - terrain_hit_mod` (被ダメージ軽減＋被命中低下を
> 合算、平地0/森林15/山25/都市35) を第一キー (降順)、次にターゲット近接・残 MP の順。「攻撃できるなら守りやすい地形から」の
> SRC 戦術判断を再現。**射程内マス選択のみ変更**で射程外からの接近 (approach) は従来どおりターゲット最接近のため既存 AI 移動/
> 接近テストへ影響なし・平地のみマップでは防御価値一様 (=0) で従来と同じ選択になるため既存 1840 テスト不変。回帰テスト 1 件
> (`ai_prefers_defensive_terrain_among_attack_tiles`)。**C 残**: 復活精神の能動 pre-buff (撃破予測要)・マップ兵器回避の散開
> (複数ユニット陣形最適化=大)。test/clippy(wasm)/wasm-check 全緑。

> **2026-06-16 追記 (続き13 — C 敵AI 完了: 復活 pre-buff＋マップ兵器散開)**: 残課題サマリ C の残り 2 件を消化し **§C を完了**。
> **復活精神の能動 pre-buff** (`e4258dd`): 瀕死 (実効最大HPの1/4以下) の自ユニットが「復活」精神を持ち SP が足りていれば撃破前に
> 自分へ「復活」を掛ける (`ai_use_revive_spirit`、ai_act_unit の free-action 段、撃破予測の近似=低HP、既保持なら温存)。
> **マップ兵器回避の散開** (`edc5992`): 敵対勢力がマップ兵器を持つとき (`hostile_has_map_weapon`)、best_move の攻撃マス選択で
> 同陣営との密集 (Manhattan≤2 の同陣営数) を最小化する第二キーを追加 (脅威無しは密集度 0 で従来挙動=既存テスト不変)。
> 両者ともテスト用ユニットは精神/マップ兵器を持たず無影響。回帰テスト 3 件 (瀕死で復活発動/健全時は温存/マップ兵器脅威下で散開)。
> **これで §C 敵AI の残課題は全消化** (残るのは「穴ではない」さらなる戦術判断のみ)。test 1844 件 / clippy(wasm) / wasm-check 全緑。

> **2026-06-16 追記 (続き14 — 「形式不確定」とされた技能 2 種を C# で確定し実装)**: 「次セッションの残」でブロッカー扱いだった
> **ハンター** (`e538bf1`) と **見切り** (`3294832`) を、`Unit.cs` 精査で仕様を確定して実装。**ハンター**: `ハンターLv*=別名 ターゲット…`
> の各ターゲット (別名=先頭トークン除く) を防御側の ユニット名/クラス/サイズ(「Lサイズ」形式)/パイロット名/性別 と照合し、一致で
> 与ダメージ ×(10+Lv)/10 (`Unit.cs::Damage` L11693、combat.rs predict の得意技/不得手 直後)。**見切り**: ロードマップの「分身看破」は誤認で、
> 実際は `Unit.cs::CheckParryFeature` L19571 の「特殊防御発動」SpecialPower＝防御側の切り払いを必ず成功 (prob=32) させる buff。roll_parry に
> 見切り condition で prob=32 上書きを追加 (SpecialPower コマンドが汎用 condition 付与するため end-to-end 機能・分身全回避の新機構は不要)。
> 回帰テスト 2 件。**教訓**: 「形式不確定/未確定」の技能は C# 一次情報 (`Unit.cs`) で確定できる — docs 未定義を理由に諦めない。
> test 1846 件 / clippy(wasm) / wasm-check 全緑。

> **2026-06-16 追記 (続き15 — 方針: シナリオ独自精神の仮実装を除去)**: ユーザ方針「エンジンは SRC_20121125/SRC.Sharp の忠実移植で
> あるべき。シナリオ独自定義の精神コマンド等はシナリオ側 (sp.txt 効果種別/イベント) で解決すべきで、原典に無い効果をエンジンへ推測実装
> しない」に従い、**決意/気迫/希望 の仮実装を除去** (`b8b277e`)。3 者とも SRC 原典 (VB6 `SRC_20121125`/C# `SRC.NET`)・`SRC.Sharp.Help` の
> 精神コマンド文書のいずれにも定義が無いことを grep で確認 (希望の唯一の doc ヒットは使用条件.md の「転載を希望」で無関係)。除去対象:
> `apply_spirit_effect` の推測効果 (決意→必中+熱血/気迫→気力+20/希望→必中+集中)・`sp_cost_for` の推測コスト (65/35/75、奇跡=75 は原典精神なので存置)・
> `morale_sufficient_for_power` の 気迫=120。SRC 標準外は `apply_spirit_effect` 既定の無害な named condition (combat.rs 非解釈) に委ね、効果・コスト・
> 必要気力はシナリオの sp.txt で解決する。これらを assert するテストは無く全 1846 テスト緑。**教訓**: 「効果不明で暫定実装」より「原典に無いなら
> 実装しない (シナリオの責務)」が正しい — 推測実装は移植の偽装になる。

VB6 製 SRC (Simulation RPG Construction) を Rust + WebAssembly に移植中。
本ドキュメントは作業継続のための要約。**解決済み課題は §9 に 1 行で要約**し、本文は
「現状・残課題・恒久リファレンス」に絞る。

**現在地（2026-06-16 セッション）**: 直近セッションの実装ログは上の「2026-06-15/16 追記 (続き7〜15)」を参照。  
**テスト**: `cargo test -p src-core` 全緑（**1846 件**）／ clippy clean（`-D warnings`）／ wasm `cargo check` OK。  
**ブランチ／コミット**: `feat/cc-attrs-recovery` を **`master` へ FF マージ＋push 済**（`origin/master` = `0de48d9`、続き1〜11 を
含む）。以降は **`master` 上で作業**（続き12〜13 = C 敵AI: 防御地形選好 `0188d19` / 復活 pre-buff `e4258dd` / マップ兵器散開
`edc5992` ＋ docs）。現在 `master` は `origin/master` から **数コミット先行**（未 push）。push はユーザの明示指示で行う（no-auto-push）。  
**§C 敵AI は全消化済**。残課題サマリの残りは外部依存・大規模・仕様未確定でこのセッションでは進められない（後述）。  
...
**次セッションの残（いずれも個別ブロッカーあり）**: ~~ハンター~~（✅ 実装済 `e538bf1` — C# で書式確定）/ ~~見切り~~（✅ 実装済
`3294832` — C# で「切り払い必中」と確定、分身全回避は誤認だった）/ 魅了·憑依（§3 意図的非対応＝設計判断）/ 習（ラーニング技
サブシステム＝技習得の新機構要・検証データ無し）/ スパロボ戦記（84MB 実機ファイル要）/ GBA 戦闘アニメ（複数セッション規模）。
> **教訓**: 「形式不確定」「未確定」とされた技能 (ハンター/見切り) は **C# (`Unit.cs`) を読めば仕様が確定**でき、両者ともこのセッションで
> 実装できた。`Unit.cs::Damage`/`CheckParryFeature` 等が一次情報。docs 未定義でも C# 原典に当たること。
...
| A5 | ✅ **精神 決意/気迫/希望 の仮実装を除去**（`b8b277e`） | シナリオ独自（東方夢想伝）で **SRC 原典 (VB6/C#)・精神コマンド文書のいずれにも定義が無い**ことを確認し、推測効果/コスト/必要気力を**除去**（方針: エンジンは原典の忠実移植、原典に無いシナリオ独自精神はシナリオ側=sp.txt 効果種別/イベントで解決すべき）。SRC 標準外は `apply_spirit_effect` 既定の無害な named condition に委ねる。✅ **捨て身の反撃時無効**（`8ec9257`）・✅ **直撃のサポートガード無効化**（`40a9bde`）・✅ **切り払い（パイロット技能、`294618d`）＋直撃の切り払い無効化**は実装済。残: 捨て身の「被弾まで継続」（lifetime 延長のみ） |
...
✅ **防御地形を選好する**（`0188d19`）: 攻撃位置 (ターゲットを射程内に収める到達マス) が複数あるとき、森/山/都市など
防御価値の高い地形を選好（`tile_defensive_value=damage_mod-hit_mod`、平地0/森林15/山25/都市35 を第一キーに best_move を変更）。
射程外からの接近は従来どおりターゲット最接近のまま（既存テスト影響なし）。
✅ **復活精神を事前発動する**（`e4258dd`）: 瀕死 (実効最大HPの1/4以下) の自ユニットが「復活」精神を持ち SP が足りていれば、
撃破される前に自分へ「復活」を掛ける（`ai_use_revive_spirit`、free action、既に保持なら再発動せず温存）。
✅ **マップ兵器の脅威下で散開する**（`edc5992`）: 敵対勢力がマップ兵器を持つとき、攻撃マス選択で同陣営との密集 (Manhattan≤2 の
同陣営数) を最小化（`hostile_has_map_weapon` でゲート、best_move の第二キー。脅威無しは従来挙動）。
**残（深掘り余地・穴ではない）**: 地形/陣形のさらなる戦術判断（防御地形以外の高度な陣形最適化等、balance 判断要・大規模）。
...
- **除去済（シナリオ独自・原典に無し）**: 決意/気迫/希望 は SRC 原典 (VB6/C#)・精神コマンド文書のいずれにも
  定義が無いことを確認し、推測実装を **除去**（`b8b277e`）。エンジンは原典の忠実移植とし、原典に無い精神は
  シナリオ側 (sp.txt 効果種別/イベント) で解決する方針。SRC 標準外は既定の無害な named condition に委ねる。
...
- **Tier 1 ✅ 決意/気迫/希望**: 原典に定義が無いシナリオ独自精神のため、推測実装を除去（`b8b277e`、§0.1・A5）。
