...
**テスト**: `cargo test -p src-core` 全緑（**1942 件**）／ clippy clean（`-D warnings`）／ wasm `cargo check` OK。  
**最新セッション（2026-06-18・`master` 直接コミット）**: 差分オラクルを **combat 予測（stage c）＋移動範囲（stage d）＋気力/精神（stage e）へ拡張**。
combat `placeattack`（命中/クリティカル/ダメージ/地形）**45/45 一致**、移動 `moverange`（平地完全一致）、気力/精神 `@morale`/`@spirit`。
過程で **実バグ 6 件**を発掘・是正（全て VB6 裏取り）: ①命中率クランプ→上限なし・最低0 ②最低ダメージ→既定10 ③地形命中修正の符号（正=防御地形）
④防御側パイロット Defense（耐久）⑤飛行/水中等の特殊移動コスト（2→1 game MP）⑥ブースト（ユニット特殊能力）の高気力ダメージ ×1.25 配線。
詳細は §1.1 stage c/d/e。**気力/精神で残る乖離（要設計判断・未着手）**: 精神の与ダメージ倍率の data 駆動化（魂/気合 等を sp.txt から・MaxDbl 非加算）。  
...
- **差分 harness のユニット/combat 状態拡張（2026-06-17）**: `tools/oracle-diff` は式層・Commands 層（逐次）に加え、
  **✅ 静的データ層（stage a-1）を構築・実バグ 1 件を是正**。新設 `oracle_loaddata` bin（Rust）が C# `LoadDataDirectory` と
  ...
  - **✅ stage e = 気力/精神（morale/spirit）diff も構築・実バグ 1 件是正（2026-06-18）**: `placeattack` に `@morale <unit> <value>`/`@spirit <unit> <name>` を追加
    ...
    **気力スケーリング自体は一致**。**★ 実バグ 1 件是正**: **ブースト（ユニット特殊能力）の高気力 ×1.25** を予測へ配線（気力 150 が cross-engine 一致 1206→1507。
    `全ユニット共通` 継承ではなく予測が `atk_unit.features` の ブースト を未参照だったのが真因＝共通能力は各ユニットに明示列挙）。
    **残（★要設計判断・未着手）**: 精神の与ダメージ倍率が Rust ハードコード（魂×3/気合×1.2）／SRC は **sp.txt データ駆動**（魂=ダメージ増加Lv15→×2.5・
    気合=気力増加で与ダメ非増・複数は **MaxDbl 非加算**）。data 駆動化は精神/SP 系（`apply_weapon_special_effects` ほか）の中規模リファクタ＝要方針決定。
  - **次フロンティア**: ① **★精神倍率の data 駆動化**（上記 stage e 残・要設計判断＝最有力の残課題）。② combat corpus 拡張（rank>0・サイズ SS/XL・極端 lv・`-`地形適応）。
