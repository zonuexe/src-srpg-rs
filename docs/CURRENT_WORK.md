...
**テスト**: `cargo test -p src-core` 全緑（**1951 件**）／ clippy clean（`-D warnings`）／ wasm `cargo check` OK。  
**最新セッション（2026-06-18・`master` 直接コミット）**: 差分オラクルを **combat 予測（stage c）＋移動範囲（stage d）＋気力/精神（stage e）へ拡張**。
combat `placeattack`（命中/クリティカル/ダメージ/地形）**45/45**、移動 `moverange`（平地完全一致）、気力/精神 `@morale`/`@spirit` **10/10**。
過程で **実バグ 8 件**を発掘・是正（全て VB6 裏取り）: ①命中率クランプ→上限なし・最低0 ②最低ダメージ→既定10 ③地形命中修正の符号（正=防御地形）
④防御側パイロット Defense（耐久）⑤飛行/水中等の特殊移動コスト（2→1 game MP）⑥ブースト（ユニット特殊能力）の高気力 ×1.25 配線
⑦攻撃側ダメージ増加精神を sp.txt データ駆動・MaxDbl 非加算へ（熱血/魂/気合）⑧防御側 被ダメージ低下も data 駆動・C# up/down-mod へ（**不屈 を 1→×0.1 是正**・鉄壁÷4同値）。詳細は §1.1 stage c/d/e。  
**★ 次セッション着手候補（ユーザ決定 2026-06-18・優先順）**: 決定論的 combat 予測オラクルの鉱脈は掘り尽くしたので、次は
**設計判断/UI を要する C 項目**へ（いずれも本セッションでは未着手＝方向付けが要る）。① **インターミッション ステータスの単機詳細化**（§1.2 B・最も着手しやすい frontend 拡張）
② **A2 着地点選択 UI**（§1.2 A・分離/発進ユニットの着地を移動範囲から対話選択。対話 UI でヘッドレス検証不可＝実機確認要）
③ **GBA クローズアップ戦闘アニメ移植**（§4 GBA・複数セッション規模・要実シナリオ検証）。
**オラクル尾部（任意・低優先）**: バリア強度吸収の精緻化／捨て身・攻撃力ＵＰ の効果統合／クリティカル技能項（超反応/超能力/底力＝要技能持ちパイロット・fixture 不足）。  
...
- **差分 harness のユニット/combat 状態拡張（2026-06-17）**: `tools/oracle-diff` は式層・Commands 層（逐次）に加え、
  **✅ 静的データ層（stage a-1）を構築・実バグ 1 件を是正**。新設 `oracle_loaddata` bin（Rust）が C# `LoadDataDirectory` と
  ...
  - **次フロンティア**: ① バリア強度吸収の精緻化・捨て身/攻撃力ＵＰ の効果統合（任意・低優先）。② combat corpus 拡張（rank>0・サイズ SS/XL・極端 lv・`-`地形適応）。
    ③ **クリティカル率の技能項**（超反応/超能力/底力。Rust `critical_probability` は素のみで技能は `App::crit_skill_bonus` に分離）。
