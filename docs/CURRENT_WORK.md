...
**テスト**: `cargo test -p src-core` 全緑（**1957 件**）／ clippy clean（`-D warnings`）／ wasm `cargo check` OK。  
**最新セッション（2026-06-18・`master` 直接コミット）**: ① ✅ **B 単機ステータス詳細（`Scene::UnitDetail`）完了**（§1.2 B）。
② ✅ **差分オラクルを combat 予測（c）＋移動（d）＋気力/精神（e）＋改造/極端 level（f）へ拡張**。combat `placeattack` 45/45・移動 `moverange` 平地一致・
気力/精神 10/10・改造/level `combat_rank_level` **20/20**。過程で **実バグ 9 件**を発掘・是正（全て VB6/C# 裏取り）: ①命中率クランプ→上限なし・最低0
...
⑨ **機体改造の武器攻撃力を乗算 `+base×10%×Rank` → C# 加算（通常+100×Rank/固据置/Ｒ・改 +50 or +10×n×Rank）へ是正**（改造ユニットの攻撃が過大だった pervasive bug）。詳細は §1.1 stage c-f。  
**★ 次セッション着手候補（優先順）**:
...
- **差分 harness のユニット/combat 状態拡張（2026-06-17）**: `tools/oracle-diff` は式層・Commands 層（逐次）に加え、
  **✅ 静的データ層（stage a-1）を構築・実バグ 1 件を是正**。新設 `oracle_loaddata` bin（Rust）が C# `LoadDataDirectory` と
  ...
  - **次フロンティア**: ① バリア強度吸収の精緻化・捨て身/攻撃力ＵＰ の効果統合（任意・低優先）。② combat corpus の更なる拡張（サイズ SS/XL・`-`地形適応＝**別 fixture が要**）。
