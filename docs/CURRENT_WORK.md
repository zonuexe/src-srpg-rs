...
- **D スパロボ戦記（進行不能）**（最優先の大課題・§2 に診断メモ）: ✅ **2026-06-16 ヘッドレス triage で核を再特定**。
  in-repo fixture を zip→`verify-archive` ドライブ＋新設 `VERIFY_VAR` で `__srcVar` 相当を観測（**ブラウザ/84MB 不要**）。
  **真の核は「敵が出撃しない」ではなく「味方が出撃しない」**＝キャラメイキングが機体にパイロットを乗せていない（味方数 0 →
  `味方平均レベル` 0除算空・「出撃できる味方がいません」で停止）。Info/式中ユーザ関数は主因でないと確認。次は **CMaking.eve を
  drive で完走させ**未実装の Info/コマンドを特定する。詳細は §2。
...
**→ 真の核は「敵出撃」ではなく「味方が出撃しない＝キャラメイキングが機体にパイロットを乗せていない」**（§2 旧仮説の「敵配置/Info 由来」は
ほぼ否定。Info も式中ユーザ関数も主因ではない）。
...
**次の一手（headless 続行可能・具体化済）**:
1. **エントリ選択の是正（要調査）**: root `スパロボ戦記/スパロボ戦記.eve` は**登録 entry #3 として存在する**が、`entrypoint::analyze` が
   `Main.eve`（戦闘）を上位にランクするため drive は戦闘から始まる。`VERIFY_ENTRY=<部分一致>` で上書き可（不一致時は登録 .eve 一覧を表示）。
   **注意点 2 つ**: (a) **日本語の env 値はシェル/nix 経由で文字化けしマッチしないことがある**（VERIFY_ENTRY="スパロボ戦記.eve" が「なし」になった）。
   ASCII で一意になる工夫か、`analyze` 側の修正が要る。(b) `analyze` のスコア変更は**全アーカイブの smoke テストに波及**するため回帰注意。
   さらに **root の `プロローグ` は Set/RenameBGM のみ（設定）で、そこからキャラメイキングへ直接遷移しない**可能性が高い → root 起点でも
   キャラメイキング入口に届くかは未検証。**まず VERIFY_ENTRY で root から駆動し、どこへ遷移するかを VERIFY_VAR/ログで観測すること**。
2. **キャラメイキング完走**: 正エントリで drive がキャラメイキングメニューに到達したら、Input に既定名・Menu に妥当選択を与えて完走させ、
   `CMaking.eve` が使う未実装の Info/関数/コマンドを `VERIFY_VAR` で特定して実装。drive の Input は現状 `respond(0)`（空）なので名前入力で要拡張。
3. 上記が通れば味方ユニットが生成され、`味方平均レベル`/`敵候補`/`敵陣営` も埋まり「敵出撃」まで一気に検証できる見込み。
