...
**残る具体的ブロッカー（次セッション）**:
1. **機体選択後の確認/ステータスメニューを drive が抜けられずループ**（`ガンダム[機体能力を確認する|決定する]` や
   `[アビリティ|属性|機体|武器]` を smoke drive の単純選択では完走できない）。`決定する` への自動進行は確認ループに陥った（autostart から除外済）。
   → drive を**キャラメイキング専用のステートフルな選択ロジック**に拡張する（名前 Input に既定値・確認は 1 回だけ 決定・ステータス閲覧は cancel で抜ける 等）か、
   ループの原因が**エンジンのメニュー応答処理にあるか**を切り分ける（`決定する` で同じメニューが再提示される点は要確認＝エンジン側の疑い）。
