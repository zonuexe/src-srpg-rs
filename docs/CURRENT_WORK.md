...
| # | 課題 | 現状 / 必要なこと |
|---|------|------------------|
| A1 | **アビリティ未対応効果** | `apply_ability_effects` の `_ => {}`: **強化**（パイロット特殊能力レベルを一定時間+。レベル管理モデルが要る）/ **能力コピー**（射程内味方へ変化。`set_unit_form` 流用＋サイズ制限/解除設定）/ **M型マップアビリティ**（複数対象。マップ攻撃の範囲機構）/ **敵対象アビリティ**（脱/除 属性。`ability_target_valid` が味方限定なので対象側判定が要る） |
| A2 | **合体/分離の精緻化** | 3機以上ルール未区別（現状は 2 マス内を全合体／3 機以上のコマンド版と 2 機の移動合体を区別せず）・**合体時のパイロット統合**（全員搭乗。現状 host のパイロットのみ。`pre_combine_pilots` を持たせて分離時に復元）・最初から合体形態の `.eve` 配置ユニットの分離（構成 uid が無い→`分離` フォームから生成が要る）・着地点は隣接自動（原典は移動範囲から選択） |
| A3 | **修理/補給の残り** | EN コスト未実装（修理装置の消費 EN）・補給は全快固定（Lv 別なし）・修理/補給の経験値は一律 10（原典は対象レベル依存） |
| A4 | **特殊効果攻撃属性の残り**（§0.5） | CC 以外の多数属性（魅/恐/狂/盲/脱/低攻低運低移/弱効剋 等）・耐性/弱点・**反撃/援護でも proc**（現状は通常攻撃の生存時のみ＝`apply_weapon_special_effects` は 1 箇所）・proc が crit を置換する原典仕様 |
