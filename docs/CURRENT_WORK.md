...
**テスト**: `cargo test -p src-core` 全緑（**1956 件**）／ clippy clean（`-D warnings`）／ wasm `cargo check` OK。  
...
**★ 次セッション着手候補（優先順）**: ① ✅ **インターミッション ステータスの単機詳細化 完了（2026-06-18・§1.2 B）**＝
`Scene::UnitDetail` を新設し味方ロスターの実効ステータスを単機表示・巡回。残る設計判断/UI 項目は —
