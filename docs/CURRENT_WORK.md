...
**★★ GBA クローズアップ戦闘アニメ＝着手済（ユーザ決定 2026-06-18・2026-06-19 Phase 2/3 進行中）**: 偵察結果・段階計画は **§4「GBA 着手準備」**に集約。
...
- **E GBA クローズアップ戦闘アニメ（★ 着手済・Phase 2/3 進行中・ユーザ決定）**: 専用バトルスプライト＋固定レイアウトの段階移植。dict 変数／戦闘アニメ Lib／`Redraw` clear 等。
  **複数セッション規模**。✅ **前提ブロッカー（GBA シナリオ無し）は誤認＝是正済**（`lib/BattleAnime*.eve` が in-repo に在る）。✅ **図形 primitive 実装済**（`dc6bdcd`）。
  残: Redraw/Keep clear 意味論・固定レイアウトのスプライト配線・実機検証。**偵察結果・段階計画は §4「GBA 着手準備」に集約**。
...
| GBA | **GBA クローズアップ戦闘アニメ移植（★ 着手済・Phase 2/3 進行中）** | 専用バトルスプライト＋固定レイアウト。✅ 前提ブロッカー（GBA シナリオ無し）は**誤認＝是正済**（スパロボ戦記 `lib/BattleAnime*.eve` が汎用戦闘アニメ Lib 相当・`try_play_battle_animation` で再生経路あり）。✅ **図形 primitive（Circle/Oval/Polygon/Arc＋FillStyle/FillColor）実装済（`dc6bdcd`）**。残: `Redraw`/`Keep` の画面クリア意味論・固定レイアウトのスプライト配線・実機検証。dict 変数（`戦闘アニメ[…]`）は既存 `prefix[key]` で表現可。**段階計画は下記「GBA 着手準備」を参照** |
...
### GBA 着手準備（着手済・2026-06-18 偵察 / 2026-06-19 Phase 2/3 進行中）
