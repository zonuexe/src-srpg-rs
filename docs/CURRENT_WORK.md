...
**現在地（2026-06-15 セッション末）**: 直近セッションの実装ログは上の「2026-06-15 追記 (続き7〜12)」を参照。  
**テスト**: `cargo test -p src-core` 全緑（**1841 件**）／ clippy clean（`-D warnings`）／ wasm `cargo check` OK。  
**ブランチ／コミット**: `feat/cc-attrs-recovery` を **`master` へ FF マージ＋push 済**（`origin/master` = `0de48d9`、続き1〜11 を
含む）。以降は **`master` 上で作業**（続き12 = 防御地形選好 `0188d19`＋docs）。現在 `master` は `origin/master` から **2 コミット
先行**（C 敵AI の feat＋docs、未 push）。push はユーザの明示指示で行う（no-auto-push）。  
...
**残（さらなる深掘り余地・balance 判断要）**: 復活精神の能動 pre-buff（撃破予測が要る）・マップ兵器回避の散開
（複数ユニットの陣形最適化＝大規模）。穴ではなく深掘り余地（§3 参照）。
