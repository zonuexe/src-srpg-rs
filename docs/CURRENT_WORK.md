...
**ブランチ／コミット**: **`feat/necessary-skill-gate`**（**本セッション 47 コミット・未 push**）。`origin/master`=`88ad16f` から先行。
...
1. **キャラメイキングの exit（headless では現状到達不可・実機確認要）**: `召喚画面` はラベル＋`Goto` ループで、**唯一の `Break`（exit）は
   ...
   **真の壁＝`LoadFileDialog` がエンジン未実装**（`grep` で src-core にヒット 0）。よって `データロード` の `読み込みファイル` は常に空 →
   `If Not 読み込みファイル = ""` 偽 → ロードブロックを丸ごとスキップ → `Break` に到達せず 召喚画面 へ戻る。**＝唯一の exit が未実装関数依存で塞がっている**。
   **次セッションのリード（実装規模あり）**: ① `LoadFileDialog` を実装（検証時は VFS の固定 `.src` パスを返す）＋ **有効な `.src` セーブ**を用意
   （`設定[パイロット一覧]` 行を持つ＝ロード対象パイロットがある）→ `パイロットリスト` Ask を cancel → `RemovePilot`（ロード分のみ削除、作成済みは残る）
   → `パイロットロード終了` → `Break` で抜ける。② or 正規プレイ（ブラウザ）で exit 操作を確認。**combat は実証済**（`VERIFY_SEAT_DEBUG`）なので
   この exit/搭乗導線は「正規プレイ通し」のための任意課題。現状 drive は目標人数を作ったら `break` で終了（出撃前まで）。
