...
**テスト**: `cargo test -p src-core` 全緑（**1961 件**）／ clippy clean（`-D warnings`）／ wasm `cargo check` OK。  
...
**★ 次セッション着手候補（優先順）**:
① **A2 着地点選択 UI**（§1.2 A・✅ 発進は実装済 `09480eb`、残は分離=複数ユニット順次配置。対話 UI の見栄えは実機確認）
...
- **A2 着地点選択**: ✅ **発進 (母艦からの出撃) は実装済（2026-06-18、`09480eb`）**: `ActionMode::LandingSelect` を新設し、`resolve_launch` が
  母艦位置を起点とする**移動範囲の空きマスから対話選択**（`db::unit_move_range_from`＋`landing_candidates`、候補無しは従来の隣接自動へフォールバック）。
  `confirm_landing` が候補クリックで配置・候補外 no-op・キャンセルで格納維持。`draw_action_overlays` が候補を黄ハイライト。native test 2 件（配置/キャンセル）。
  **クリック選択の見栄えは実機確認**（対話 UI）。スクリプト/AI の `Launch` コマンドは非対話のまま。
  **残**: ① **分離 (合体形態) の着地点選択**（複数構成ユニットの順次配置＝LandingSelect のキュー化が要る・現状は隣接自動）。
  ② 初期合体形態の分離後の再合体トラッキング・分離パイロットの主/副の細分配分（現状は形態 1 へ集約）。
