...
> (複数セッション)。さらに **C 敵AI深掘りに一部着手**: AI が攻撃前に攻撃補助精神(魂/熱血/必中)を使う `2589471` /
> ChangeMode「逃亡」を尊重して逃走 `38dad3a`。ブランチ `feat/cc-attrs-recovery`、全テスト緑。push/マージはユーザ指示待ち。
...
### C. 敵 AI の深掘り（中〜大）— 一部着手

AI は機能している（`ai_runner_tick`/`ai_act_unit`）が浅い。一部を深掘り済:
✅ **攻撃補助精神を使う**（攻撃前に習得済みの 魂/熱血/必中 を発動。`ai_use_offensive_spirit`、`2589471`）。
✅ **ChangeMode「逃亡」を尊重**（恐怖と同じ逃走分岐へ。`38dad3a`）。
**残（深掘り余地）**: アビリティ（回復/補給/召喚）を使わない・マップ兵器を使わない・`ChangeMode`「護衛」を無視。
穴ではなく深掘り余地（§3 参照）。
