...
- **差分 harness のユニット/combat 状態拡張（2026-06-17）**: `tools/oracle-diff` は式層・Commands 層（逐次）に加え、
  **✅ 静的データ層（stage a-1）を構築・実バグ 1 件を是正**。新設 `oracle_loaddata` bin（Rust）が C# `LoadDataDirectory` と
  ...
  - **次フロンティア**: ① SetStatus/状態異常の diff は**式層では低価値**と判明（condition の stat 効果は `effective_combat_data`＝combat 経路のみで、
    `Info(ユニット,装甲/運動性)`＝`effective_*` には乗らない＝presence 確認に留まる）。② **combat 予測（命中/ダメージ/クリティカル率）の diff** が
    次の本命（決定論的＝RNG ロール前の予測値。ただし両エンジンとも式関数で露出していないため、attacker/defender を立てて予測メソッドを直接呼ぶ
    専用ハーネスが要る＝セッション級）。生成スキャフォルド（無人/有人ユニット実体化）は stage a-2/b で確立済。設計は memory `reference_csharp_oracle`。
