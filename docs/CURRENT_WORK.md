...
**テスト**: `cargo test -p src-core` 全緑（**1899 件**）／ clippy clean（`-D warnings`）／ wasm `cargo check` OK。  
**ブランチ／コミット**: **`feat/necessary-skill-gate`**（**本セッション 71 コミット・未 push**）。`origin/master`=`88ad16f` から先行。
...
- **差分 harness のユニット/combat 状態拡張（2026-06-17 着手・調査中）**: `tools/oracle-diff` は式層・Commands 層（逐次）を
  C#↔Rust で diff 済。次フロンティアは**ユニット/combat 状態の diff**。**ブロッカー確定**: Rust の `.eve Pilot`/`Unit` の 12 フィールド
  inline 定義は Rust 独自拡張で、SRC は拒否（`PilotCmd.cs:18` ＝ 3-4 引数・arg2 は pilot.txt 事前定義名を `PDList.IsDefined` 参照）。
  → 両エンジンで同一ユニットを作るには **pilot.txt/unit.txt のデータロードが前提**。
  **C# 経路（調査済・一部実証）**: `SRC.LoadDataDirectory(fpath)`（`SRC.data.cs:52`）が `pilot.txt`→`PDList.Load` /
  `unit.txt`(robot.txt)→`UDList.Load` をロード。**✅ headless 実行可を実証**: `src.FileSystem = new LocalFileSystem();`
  `src.LoadDataDirectory(dir)` が in-repo の `fixtures/スパロボ戦記/data/スパロボ戦記`（Shift-JIS）を例外なくロードし
  PDList/UDList を populate（`tools/oracle-diff` の `loaddata` モードで確認）。**残ブロッカー（確定）**:
  ① ユニットクエリ（`Info`/`HP`/`MaxHP`）は **PLACED ユニット（UList）** を読むため、データロードだけでは解決しない
  → **map 初期化 ＋ `Create`/`Place` コマンド**が必要（`CreateCmd` は GUIMap を要し最小 SRC では不可）。
  ② UDList が少件数 = `IncludeData`（scenario の Include で複数 data dir を合成）未通過 → フル検証は scenario ロード経路が要る。
  ③ SRC `Pilot` コマンドは 3-4 引数で arg2 が `PDList.IsDefined` 参照（`PilotCmd.cs:18`）。
  ④ combat 状態まで行くなら **RNG シード一致**。
  **結論**: データロードの土台は動くが、ユニット状態 diff には headless game-init（map+配置）が要る**セッション級統合**。
  **段階計画**: (a) データ Include + map 初期化 + Place を headless で通す → 配置ユニットの静的状態 diff、
  (b) 決定論的な状態変更（ExpUp/Damage/Heal/SetStatus）の diff、(c) combat（RNG 一致）の diff。設計は memory `reference_csharp_oracle`。
...
- **乖離記録**: `docs/SRC_SHARP_DIVERGENCE.md` §2-4 ＋乖離候補に、Round（SRC.Sharp が VB6 から乖離）・Not 優先順位・是正済み旧乖離・
  Set-& 寛容差・Pilot/Unit inline 形式（Rust 独自拡張）を記録。
- **テスト**: `cargo test -p src-core` **1934 件**全緑 / clippy `-D warnings` / wasm check OK。
