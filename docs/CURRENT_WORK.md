...
1. **キャラメイキングの exit（headless では現状到達不可・実機確認要）**: `召喚画面` はラベル＋`Goto` ループで、**唯一の `Break`（exit）は
   ...
   **次セッションのリード**: ① `データロード` をヘッドレスで成立させる（VFS に最小 `.src` セーブを置く or `LoadFileDialog` を検証時に固定パス返す）→
   `パイロットリスト` Ask を cancel すれば作成済みパイロットを残したまま `Break` で抜けられるはず。② or 正規プレイ（ブラウザ）で exit 操作を確認
   （右クリック再登録は本実装のメニュー右クリック処理が原典と乖離している疑いもある→`KeyState(2)`/`ClearObj`/`Continue` の意味論を要照合）。
   現状 drive は目標人数を作ったら `break` で終了（出撃前まで）。
