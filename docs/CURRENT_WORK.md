...
- **(B) `DefenseMode` を本番配線して確率ロールを正式導入（将来）**: 命中ロール後の回避ロール、
  ダメージ確定後の吸収/半減ロールを戦闘実行フローへ差し込む。シールド発動率は **メインパイロットの
  S防御能力依存**（`Unit.cs:20519`）なので S防御値の算出経路も要設計。整合する oracle 回帰を付ける。
