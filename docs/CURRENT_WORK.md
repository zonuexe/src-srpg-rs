...
> 確認。`9756f65`）。回帰テスト 3 件。test/clippy/wasm-check 全緑。次は A4 耐性/弱点 → A2 残（着地点選択
> 等）→ 残る CC 属性 の順を予定。
