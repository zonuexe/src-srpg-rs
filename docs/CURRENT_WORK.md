...
- **差分 harness のユニット/combat 状態拡張（2026-06-17 着手・調査中）**: `tools/oracle-diff` は式層・Commands 層（逐次）を
  ...
  **C# 経路（調査済）**: `SRC.LoadDataDirectory(fpath)`（`SRC.data.cs:52`）が `pilot.txt`→`PDList.Load` / `unit.txt`(robot.txt)→`UDList.Load` /
  `sp.txt`/`non_pilot.txt` をロードする。**未調査**: ① `LoadDataDirectory` の headless 実行可否（`FileSystem` 抽象・Shift-JIS）、
  ② SRC `Pilot`/`Place` コマンドの正規シグネチャと Rust との互換、③ 両エンジンで使える in-repo シナリオの pilot.txt/unit.txt の所在、
  ④ combat まで行くなら **RNG シード一致**（両実装の乱数を揃える）。**セッション級の統合**。設計と前提は memory `reference_csharp_oracle` 末尾。
