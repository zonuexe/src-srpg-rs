...
1. **エントリ選択の是正**: verify-archive に `VERIFY_ENTRY=<file>` 等で entry .eve を上書きできるようにし、root スパロボ戦記.eve から駆動。
   ブラウザ（src-web archive.rs）のエントリ選択ロジックと verify-archive `entrypoint::analyze` の差を埋める（root をシナリオ名で優先）。
2. **キャラメイキング完走**: 正エントリで drive がキャラメイキングメニューに到達したら、Input に既定名・Menu に妥当選択を与えて完走させ、
   `CMaking.eve` が使う未実装の Info/関数/コマンドを `VERIFY_VAR` で特定して実装。
