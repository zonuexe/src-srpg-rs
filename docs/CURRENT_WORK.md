...
- **キャンバス拡張 768×480 / 右パネル 288px**（`f110ed0`）: 当初「640×480維持」だったが、他完了後にユーザ判断で拡張。`CANVAS_WIDTH` 640→768 / `STATUS_PANEL_WIDTH` 160→288（マップ領域は 480×480 のまま、`MAP_VIEW_WIDTH`=768=`CANVAS_WIDTH` で ox=0）。全画面シーン（UnitDetail/Intermission）は幅を `CANVAS_WIDTH` 参照にして ox=0 維持（描画/クリック判定のオフセットずれ回避）。武器名等の切り詰め幅を緩和し**フル表示**。戦闘窓は CANVAS_WIDTH 中央寄せで自動再配置。

**到達点**: オリジナルの戦闘窓**4種**（①メッセージ②反撃③武器選択④反撃武器選択）＋ステータスパネル（288px・武器名フル表示・タイプ行）が揃った。実機で各窓・クリック確定・反撃武器チェーン・拡張後レイアウトを目視確認済み（タイプのみ実機未確認のまま拡張で確認できた）。

**残（次セッション候補・低優先・データ要）**:
1. **戦闘窓・パネルの実顔グラ確認**: デモ機に立ち絵アセットが無くプレースホルダ。実シナリオでの確認は実機要。
2. **UnitDetail 内容の 768 幅最適化**: 全画面化したが内容レイアウトは 640 幅基準のままで右に余白（破綻なし・微調整余地）。
