...
**テスト**: `cargo test -p src-core` 全緑（**1937 件**）／ clippy clean（`-D warnings`）／ wasm `cargo check` OK。  
**ブランチ／コミット**: 直近は **`feat/oracle-unit-diff`**（差分オラクルのデータ層＋ユニット/パイロット実体層拡張）。`master`=`575c38f` から先行・未 push。
...
- **差分 harness のユニット/combat 状態拡張（2026-06-17）**: `tools/oracle-diff` は式層・Commands 層（逐次）に加え、
  **✅ 静的データ層（stage a-1）を構築・実バグ 1 件を是正**。新設 `oracle_loaddata` bin（Rust）が C# `LoadDataDirectory` と
  ...
  - **★★ 大物バグを発掘（高優先・未是正＝次タスク）: パイロットのレベル成長式が SRC と大きく乖離**。Rust `grown_pilot`/
    `apply_stat_growth` は class ベース `base+(level-1)*rate`（過大成長）。SRC（VB6 `Pilot.cls:582-593`）は **`lv=Level`（レベル 1 でも成長）・
    格闘/射撃/技量/反応 +=level・命中/回避 +=2*level**。オラクル実測: 人工知能 lv10 格闘 C#=110/Rust=190、超人工知能 lv30 C#=155/Rust=415。
    **全レベルアップ済みパイロットの戦闘実効値に波及する pervasive bug**。是正は両関数を VB6 式へ＋「level 1 = base」前提（`from_data` 等）の
    見直し＋戦闘テスト期待値更新を伴う集中作業（回帰検証は `unit_pilot.txt` の 10 growth probe）。詳細・正確な式は `docs/SRC_SHARP_DIVERGENCE.md`。
  - **次フロンティア**: ① **パイロット成長式の是正**（上記・最優先）→ ② SetStatus/状態異常の diff → ③ combat（RNG 一致）の diff。
    生成スキャフォルド（無人/有人ユニットの実体化）は stage a-2/b で確立済。設計は memory `reference_csharp_oracle`。
...
- **差分 harness を有人ユニット（パイロット実体）へ拡張（stage b、`@unit` 5 フィールド有人形式）**: コーパス `unit_pilot.txt`。**実バグ 1 件を
  ...
  **★ 大物バグを発掘（高優先・未是正）**: パイロットのレベル成長式が SRC と大きく乖離（Rust=class ベース過大成長 / SRC=`lv=Level`・格闘等 +level・
  命中/回避 +2*level）。全レベルアップ済みパイロットに波及。詳細・正確な式は `docs/SRC_SHARP_DIVERGENCE.md`（次タスクで是正）。
- **乖離記録**: `docs/SRC_SHARP_DIVERGENCE.md` §2-4 ＋乖離候補に、Round（SRC.Sharp が VB6 から乖離）・Not 優先順位・是正済み旧乖離
  （技量/反応 取り違え・Create rank/level 含む）・Set-& 寛容差・Pilot/Unit inline 形式・性別/クラス別名/全ユニット共通/気力 の各乖離・
  **パイロット成長式（★高優先・要是正）**を記録。
