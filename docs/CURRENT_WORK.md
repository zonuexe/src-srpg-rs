...
**テスト**: `cargo test -p src-core` 全緑（**1936 件**）／ clippy clean（`-D warnings`）／ wasm `cargo check` OK。  
**ブランチ／コミット**: 直近は **`feat/oracle-unit-diff`**（差分オラクルのデータ層＋ユニット実体層拡張）。`master`=`575c38f` から先行・未 push。
...
- **差分 harness のユニット/combat 状態拡張（2026-06-17）**: `tools/oracle-diff` は式層・Commands 層（逐次）に加え、
  **✅ 静的データ層（stage a-1）を構築・実バグ 1 件を是正**。新設 `oracle_loaddata` bin（Rust）が C# `LoadDataDirectory` と
  ...
  - **次フロンティア**: (b) 決定論的状態変更（ExpUp/Damage/Heal/SetStatus）の diff → (c) combat（RNG 一致）の diff。
    生成スキャフォルド（両エンジンのユニット実体化）は stage a-2 で確立済。設計は memory `reference_csharp_oracle`。
...
- **乖離記録**: `docs/SRC_SHARP_DIVERGENCE.md` §2-4 ＋乖離候補に、Round（SRC.Sharp が VB6 から乖離）・Not 優先順位・是正済み旧乖離
  （技量/反応 取り違え・Create rank 含む）・Set-& 寛容差・Pilot/Unit inline 形式（Rust 独自拡張）・性別/クラス別名/全ユニット共通/気力 の 4 乖離を記録。
- **テスト**: `cargo test -p src-core` **1936 件**全緑 / clippy `-D warnings` / wasm check OK。
