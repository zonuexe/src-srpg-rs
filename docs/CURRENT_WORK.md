...
**テスト**: `cargo test -p src-core` 全緑（**1962 件**）／ clippy clean（`-D warnings`）／ wasm `cargo check` OK。  
...
**★★ 次セッション＝GBA クローズアップ戦闘アニメに着手（ユーザ決定 2026-06-18）**: 着手準備の偵察結果・前提ブロッカー・段階計画は **§4「GBA 着手準備（次セッション起点）」**に集約。
最重要: ① **前提ブロッカー＝in-repo に GBA シナリオが無い**（`_GBA_*`/汎用戦闘アニメ Lib を含む fixture が皆無＝まず**テスト用 GBA シナリオの入手**が第一手）。② エンジンは描画（PaintString/PaintPicture/Line/PSet/Color/Cls）/配列(dict)変数/Redraw/Call/script_overlay 独自画面 の素材は揃う。
③ GBA クローズアップは**エンジン機能ではなくシナリオ側 汎用戦闘アニメ `.eve` サブルーチン**で、engine 側は primitives（`_GBA_*` が使う命令＋Redraw/Keep clear 意味論＋固定レイアウト画面）を満たす形。
...
- **E GBA クローズアップ戦闘アニメ（★ 次セッション着手・ユーザ決定）**: 専用バトルスプライト＋固定レイアウトの段階移植。dict 変数／`_GBA_*`／`Redraw` clear 等。
  **複数セッション規模・要実シナリオ検証**。**偵察結果・前提ブロッカー（in-repo に GBA シナリオ無し）・段階計画は §4「GBA 着手準備（次セッション起点）」に集約**。
...
| GBA | **GBA クローズアップ戦闘アニメ移植（★ 次セッション着手）** | 専用バトルスプライト＋固定レイアウト。`BaseX/BaseY=0` 固定画面に `_GBA_GetUnitBmpFile(UID,…)` でユニット個別スプライトを描く。dict 変数（`戦闘アニメ変数[…]`/`_GBA[…]`）＋数十の `_GBA_*`＋`Redraw`/`Keep` の画面クリア意味論依存。**複数セッション規模・要実シナリオ検証**。**偵察結果＋前提ブロッカー（in-repo に GBA シナリオ無し）＋段階計画は下記「GBA 着手準備（次セッション起点）」を参照** |
...
### GBA 着手準備（次セッション起点・2026-06-18 偵察）
...
- doc §4 が挙げる `_GBA_GetUnitBmpFile(UID,…)`・`戦闘アニメ変数[…]`/`_GBA[…]`・数十の `_GBA_*`・`Redraw`/`Keep` は
  **汎用戦闘アニメ Lib のサブルーチン/変数**。`_GBA_*` は engine 関数ではなく **シナリオラベル（`Call(_GBA_…)`）** の可能性が高い。

**★ 前提ブロッカー（第一手）**: **in-repo の fixture に GBA クローズアップを使うシナリオが無い**
（`grep -r _GBA_ / 汎用戦闘アニメ / クローズアップ` が fixtures で 0 件）。スパロボ戦記/テイルズの `animation.txt` は
**標準の戦闘アニメデータ（汎用戦闘アニメではない）**。→ **まず汎用戦闘アニメ Lib を含むテスト用 GBA シナリオを入手**しないと
ヘッドレス検証も実機検証もできない。これが次セッションの最初のタスク。

**engine 側の現状（素材は概ね揃う）**:
- ✅ 描画命令: `PaintString`/`SystemPaintString`/`PaintPicture`/`Line`/`PSet`/`Color`/`Cls` = Implemented（`command_catalog.rs:303-339`）。
  `Arc`/`Circle`/`Oval`/`Polygon` = Stub。
...
**段階計画（次セッション）**:
1. **(前提) GBA シナリオ入手** → `verify-archive` でヘッドレス駆動し、戦闘発生時に**未対応で落ちる命令/関数/変数を洗い出す**（gap の実測）。
   既存の `VERIFY_*` ドライブ＋対話発生元計装（`App.exec_pc` 逆引き）で、どの汎用戦闘アニメ命令が未実装/Stub かを特定。
2. **primitives 充足**: 洗い出した未対応命令（描画/`Redraw`/`Keep` clear/dict 操作 等）を実装。特に **Redraw/Keep の画面クリア意味論**が要。
3. **固定レイアウト描画の配線**（src-web）: `BaseX/BaseY=0` の固定画面に script_overlay 経由でユニット個別スプライトを描く。ユニット解決（`_GBA_GetUnitBmpFile` 相当）。
4. **実機検証**（対話/描画ゆえ headless 不可の見栄え確認）。
