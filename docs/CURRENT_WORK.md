...
**現在地（2026-06-15 セッション末）**: 直近セッションの実装ログは上の「2026-06-15 追記 (続き7〜9)」を参照。  
**テスト**: `cargo test -p src-core` 全緑（**1835 件**）／ clippy clean（`-D warnings`）／ wasm `cargo check` OK。  
**ブランチ／コミット**: feature ブランチ **`feat/cc-attrs-recovery`**（master から **58 コミット**先行、最終コミット
`9b8cb4f`）。**全てコミット済・master 未マージ・push 待ち**（CLAUDE.md の no-auto-push 規約によりユーザの明示指示で
マージ＋push する）。  
**本セッション（2026-06-15）で実装（16 feature + 6 docs）**:  
...
- **気力経済を完全実装（最重要）**: 撃墜（撃破者+4·同陣営+1·機械不動）＋被撃破陣営の性格別変動（超強気+2/弱気-1）＋
  被弾+1 ＋ 命中時/損傷時/失敗時/回避時 気力増加スキル。→「戦って気力を上げ→精神/底力/潜在力開放が発動」する
  SRC の中核ループが機能するようになった。  
**次セッションの残（いずれも個別ブロッカーあり、§A4 末尾・続き9 参照）**: ハンター（対象カテゴリ別ダメージ＝技能データ
書式が docs 未定義・検証シナリオ無し）/ 見切り（分身の確率全回避システム要・RNG 列退行リスク）/ 魅了·憑依（§3 意図的
非対応＝設計判断）/ 習（必要技能ゲート＝検証データ無し）/ スパロボ戦記（84MB 実機ファイル要）/ GBA 戦闘アニメ（複数
セッション規模）。
