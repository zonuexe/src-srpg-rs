...
**テスト**: `cargo test -p src-core` 全緑（**1939 件**）／ clippy clean（`-D warnings`）／ wasm `cargo check` OK。  
**最新セッション（2026-06-18・`master` 直接コミット）**: 差分オラクルを **combat 予測層（stage c）へ拡張**し `placeattack` モードを新設、
命中率/クリティカル率（`combat_prediction.txt`）＋ダメージ（`combat_damage.txt`）を C# と突合して **32/32 cross-engine 一致**。
過程で **pervasive 実バグ 2 件**を発掘・是正（**命中率クランプ** `clamp(5,95)`→上限なし・最低 0／**最低ダメージ** `max(1)`→既定 10、いずれも VB6 裏取り済）。
詳細は §1.1 stage c。  
...
- **差分 harness のユニット/combat 状態拡張（2026-06-17）**: `tools/oracle-diff` は式層・Commands 層（逐次）に加え、
  **✅ 静的データ層（stage a-1）を構築・実バグ 1 件を是正**。新設 `oracle_loaddata` bin（Rust）が C# `LoadDataDirectory` と
  ...
  - **✅ stage c = combat 予測（命中率/ダメージ/クリティカル率）diff も構築・実バグ 2 件是正（2026-06-18）**: 専用モード `placeattack`
    ...
    **★★ pervasive 実バグ 2 件を発掘・是正**: ① **命中率クランプ**＝旧 `clamp(5,95)`（他 SRPG 慣習）を VB6 `Unit.cls:6694-6696` 準拠の
    **上限なし・最低 0**（`combat.rs`、>100=必中、表示のみ `min(100)`）。② **最低ダメージ**＝旧 `max(1)` を VB6 `Unit.cls:7460` 準拠の**既定 10**。
    いずれも全戦闘の命中/ダメージに波及。SRC ダメージ式 `(攻撃力−防御力)×地形ダメージ修正` は**構造が Rust と同一**で、上記 2 下限が唯一の乖離だった
    （攻撃力にも武器/ユニットの地形適応が乗ることも実数で確認＝`戦闘システム詳細.md` 準拠）。回帰テスト 2 件（`hit_chance_has_no_upper_cap`/`minimum_damage_is_ten`）。
  - **次フロンティア**: ① **combat 予測の corpus 拡張**（rank>0 の改造ユニット・サイズ SS/XL・極端レベル lv1/lv99・`-`地形適応＝Rust 0.0/C# 0.6 fallback の乖離候補
    をダメージ経路で検証）。② **地形を変えた diff**（terrain.txt をロードし防御地形に配置＝地形 hit_mod/damage_mod＋非中立 adaptation。Rust hit は `(100+hit_mod)`・
    C# は `(100-HitMod)` で符号規約が逆＝格納側の符号で相殺されるか要検証）。③ **状態異常/気力/精神を載せた予測 diff**（熱血/魂/集中 等。C# 側は spirit を立てる手間）。
