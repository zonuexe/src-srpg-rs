...
**テスト**: `cargo test -p src-core` 全緑（**1946 件**）／ clippy clean（`-D warnings`）／ wasm `cargo check` OK。  
...
過程で **実バグ 7 件**を発掘・是正（全て VB6 裏取り）: ①命中率クランプ→上限なし・最低0 ②最低ダメージ→既定10 ③地形命中修正の符号（正=防御地形）
④防御側パイロット Defense（耐久）⑤飛行/水中等の特殊移動コスト（2→1 game MP）⑥ブースト（ユニット特殊能力）の高気力 ×1.25 配線
⑦攻撃側ダメージ増加精神を sp.txt データ駆動・MaxDbl 非加算へ（熱血/魂/気合）。詳細は §1.1 stage c/d/e。  
  ...
  **✅ 静的データ層（stage a-1）を構築・実バグ 1 件を是正**。新設 `oracle_loaddata` bin（Rust）が C# `LoadDataDirectory` と
  ...
  - **✅ stage e = 気力/精神（morale/spirit）diff も構築・実バグ 1 件是正（2026-06-18）**: `placeattack` に `@morale <unit> <value>`/`@spirit <unit> <name>` を追加
    ...
    **気力スケーリング自体は一致**。**★ 実バグ 2 件是正**: ⑥ **ブースト（ユニット特殊能力）の高気力 ×1.25** を予測へ配線（気力 150 が 1206→1507。
    ...
    ⑦ **攻撃側ダメージ増加精神を sp.txt データ駆動・MaxDbl 非加算へ**（`SpecialPowerData.effects`＋`db::sp_damage_increase_level`＋`predict` の
    `atk_damage_boost_level`）。熱血+魂=×2.5(MaxDbl)・気合=×1.0（気力増加で与ダメ非増）で **morale corpus 10/10 一致**。捨て身/攻撃力ＵＰ/防御側
    被ダメージ低下（鉄壁/不屈/バリア）は範囲外で従来どおり（double-application 回避）。
  - **次フロンティア**: ① 防御側 被ダメージ低下/増加 の data 駆動化（鉄壁/不屈/バリア。捨て身/攻撃力ＵＰ の効果統合も。任意・低優先）。② combat corpus 拡張（rank>0・サイズ SS/XL・極端 lv・`-`地形適応）。
