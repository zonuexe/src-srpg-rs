...
**ブランチ／コミット**: **`feat/necessary-skill-gate`**（**本セッション 66 コミット・未 push**）。`origin/master`=`88ad16f` から先行。
...
次セッションの残課題は §1。**★ ユーザ決定（2026-06-16）**: ① **魅了/憑依は「spec 準拠で実装する」方針に確定**（従来の「意図的非対応」を解除。
未着手・実装計画は §1.1）。② 並行方針は **「更にバグ探索を継続」**（in-repo fixture は全駆動済のため、新規の実シナリオ zip を各自取得して
...
- **魅了/憑依**: ✅ **ユーザ決定（2026-06-16）＝「spec 準拠で実装する」方針に確定（未着手）**。従来「§4 で意図的非対応」としてきたが、
  原典 SRC 仕様（`特殊効果攻撃属性.md` 69-75 魅 / 113-117 憑）に定義済みのため**忠実移植として実装する**判断に変更。勢力/支配の切替で
  勝敗・ターン・AI・save と広く干渉するため、ChangeParty の `u.party=` 既存基盤を再利用して**慎重・最小**に実装する。**実装計画**:
  - **検出ヘルパ** `combat::weapon_possession(class) -> Option<&'static str>`（`憑`→"憑依" / `魅`→"魅了"、`split_attr_level` で class トークン走査）。
  - **フィールド** `UnitInstance.charm_revert_party: Option<Party>`（`#[serde(default)]`）＝魅了で一時勢力変更したときの**元勢力**を保持。
    全構造体リテラル構築点に `None` を補う（コンパイルエラーで漏れを検出）。
  - **配線** `App::apply_weapon_special_effects`（`app.rs:5570`）の proc 成功分岐へ。**early-return 条件**（`effects.is_empty() && morale_down.is_none()`）に
    possession の有無も加える。`is_boss`（BossRank）と firer の party/uid を取得:
    - **憑（憑依）**: 相手を乗っ取り**恒久支配**。`!is_boss` のとき `unit_instances[def_idx].party = firer_party`。**BossRank 適用ユニットは無効**。
      condition `憑依`（永続）を付与し SpecialPower 使用を封じる（spec「スペシャルパワーは使用できません」）。**復帰なし**。
    - **魅（魅了）**: 相手を **3 ターン魅了**。`!is_boss` のとき `charm_revert_party=Some(旧party)`・`party=firer_party`・`ai_mode="護衛 <firer_uid>"`
      （`ai_act_unit` の護衛分岐＝魅了主の近くへ移動し射程内の敵を攻撃＝spec「魅了主を護衛する味方のように行動」）・condition `魅了` lifetime=4
      （3 ターン＋フェイズ開始 tick 吸収＝roadmap の「lifetime=ターン数+1」規約）。
  - **復帰フック** `begin_phase`（`app.rs:2014`、`tick_conditions` の**後**）に追加: `charm_revert_party.is_some()` かつ `魅了` condition が消えていれば、
    `party` を `charm_revert_party` から復帰し、設定した `ai_mode`（護衛）と `charm_revert_party` をクリア。
  - **干渉点（要テスト）**: 勝敗判定（`check_victory`）・ターン/フェイズ・AI 標的（`is_hostile_to` 再評価）・save/load（serde default）。
    synthetic test = 憑依（恒久・ボス免疫）/ 魅了（一時・期限切れ復帰）。**注**: in-repo fixture に 憑/魅 武器が無いため synthetic で検証（§3 ゲートと同様）。
    必中 condition で hit_chance=100・proc 率を上げるか RNG を固定して決定論化する（既存の特殊効果テストと同じ手法）。
...
| 魅了/憑依 | **勢力/支配の切替** | ✅ **ユーザ決定（2026-06-16）＝spec 準拠で実装する方針に確定（未着手）**。魅了=3T 一時・魅了主を護衛する味方として行動・期限切れ復帰 / 憑依=恒久支配・BossRank 免疫・SpecialPower 封じ。ChangeParty の `u.party=` 基盤を再利用して慎重・最小に実装。**詳細な実装計画は §1.1** |
