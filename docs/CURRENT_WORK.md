...
**テスト**: `cargo test -p src-core` 全緑（**1958 件**）／ clippy clean（`-D warnings`）／ wasm `cargo check` OK。  
...
気力/精神 10/10・改造/level `combat_rank_level` **20/20**・サイズ差 `combat_size_tales` **7/7**。過程で **実バグ 10 件**を発掘・是正（全て VB6/C# 裏取り）: ①命中率クランプ→上限なし・最低0
...
⑩ **散（散布）属性武器の距離補正を実装**（manhattan 距離 1-5+ で 命中 +0/+5/+10/+15/+20・ダメージ ×1.0/0.95/0.90/0.85/0.80。ライブ戦闘 3 経路＋プレビューに配線。Rust は 散 を一切未実装だった）。詳細は §1.1 stage c-g。  
**★ 次セッション着手候補（優先順）**:
① **A2 着地点選択 UI**（§1.2 A・分離/発進ユニットの着地を移動範囲から対話選択。対話 UI でヘッドレス検証不可＝実機確認要）
② **GBA クローズアップ戦闘アニメ移植**（§4 GBA・複数セッション規模・要実シナリオ検証）。
...
- **差分 harness のユニット/combat 状態拡張（2026-06-17）**: `tools/oracle-diff` は式層・Commands 層（逐次）に加え、
  **✅ 静的データ層（stage a-1）を構築・実バグ 1 件を是正**。新設 `oracle_loaddata` bin（Rust）が C# `LoadDataDirectory` と
  ...
  - **★ 要調査（次セッション最有力・防御特性 弱点/吸収のダメージ計算）**: 別 fixture（テイルズ モンスター、`弱点=火`/`吸収=水`/`耐性=闇` 多数）で防御特性を突合しようとして、**spec と Rust 実装の乖離**を発見したが
    **オラクル harness では確認しきれない経路差**に当たった。**spec（`防御特性に関する特殊能力.md`、権威）**: 弱点=**装甲半減**（被ダメ増）/吸収=（装甲無視ダメージ）÷2 回復/耐性=÷2/無効化=0/有効=吸収・無効化・耐性を打ち消す。優先 **弱点>有効>吸収>無効化>耐性**。
    **C#**: `Unit.Damage(w,t,is_true_value,…)` の `Unit.cs:11192-11205` で **`is_true_value | mpskill>=140` のとき** `t.Weakness(wclass)`→`arm/=2`・`t.Absorb`→`arm=0`（＝spec の装甲半減/無視を実装）。クリ率は `12400` で 弱点 +10/耐性 ÷2（`adjust_proc_for_resistance` 相当）。
    **Rust**: `app.rs::defense_attribute_damage` は **弱点/有効＝ダメージ変化なし**（耐性÷2・無効化0・吸収=最終ダメージ÷2 は実装）。**＝spec 準拠なら 弱点 で装甲半減すべきだが未実装**（ほぼ全モンスターが弱点持ち＝pervasive の可能性大）。吸収も「装甲無視ダメージ」基準でなく最終ダメージ基準で乖離疑い。
    **未解決**: oracle harness は `WeaponData.Damage(defender,true)` を呼ぶが、これは `Unit.Damage(idx,t,is_true_value=true)`（11193 の arm/2 経路）と**別実体らしく**、紅蓮(格実火) vs バグベア(弱点=火) L99 でも **C#==Rust（弱点効果なし）**＝forecast 経路は一致。`WeaponData.Damage` の委譲先（is_true_value を 11193 に渡すか）が未確認。**次手**: harness を `Unit.Damage(idx,t,true)` 直呼びに変えて弱点が C# で効くか確認 → 効けば Rust 側を「武器属性が defender の弱点に一致時 def 装甲を予測前に半減」へ是正（synthetic で spec をピン）。**core ダメージ式の変更につき oracle 確認後に着手**（推測実装はしない方針）。
  - **次フロンティア**: ① バリア強度吸収の精緻化・捨て身/攻撃力ＵＰ の効果統合（任意・低優先）。**Ｓ防御＝シールド防御**（`Unit.cs:20519`、シールド特殊能力持ちが確率で被ダメージブロック）も
    ...
    ④ 移動 corpus 拡張（水中/宇宙専用 passability＝該当 fixture ユニット不足／ZoC・遮蔽）。
