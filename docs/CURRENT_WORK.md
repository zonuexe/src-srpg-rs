...
**テスト**: `cargo test -p src-core` 全緑（**1846 件**）／ clippy clean（`-D warnings`）／ wasm `cargo check` OK。  
**ブランチ／コミット**: `feat/cc-attrs-recovery` を `master` へ FF マージ＋push 済（`origin/master` = `0de48d9`）。
以降は **`master` 上で作業**し、現在 `master` は `origin/master` から数コミット先行（未 push）。push はユーザの明示指示で行う（no-auto-push）。
...
- **必要技能ゲート（＋依存する 習）**: SRC の `(念力Lv3)` 形式の括弧条件で武器/アビリティ/特殊能力の使用可否を制限する機能。
  現状 `必要技能` は extras 格納＋Info 参照のみで `is_weapon_available` のゲートに**未配線**（event_runtime.rs L10301）。
  括弧条件のパーサ＋評価器の**新サブシステム**で、全実シナリオの武器可否に波及・検証シナリオ無し → 誤実装は武器を誤封印
  （＝不忠実）。**習（ラーニング技、クリティカルで相手の技を習得）はこのゲートが前提**のため単独実装は no-op。設計判断＋検証データ要。
...
- **B インターミッション**: ステータスの単機詳細化（現状はロスター閲覧の MVP）・乗り換えの Option ゲーティング（`Option` コマンド
  未対応のため「2 機以上」で代替中）。いずれも MVP 拡張。
...
| 必要技能 | **必要技能ゲート（＋習）** | `(念力Lv3)` 形式の括弧条件で武器/アビリティ可否を制限。現状 extras 格納のみで未配線。新サブシステム・広範囲・検証無し。習（ラーニング）はこれが前提（§1.1） |
