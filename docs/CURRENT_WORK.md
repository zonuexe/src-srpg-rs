...
> (複数セッション)。**C 敵AI深掘り — §3 の主要ギャップを解消**: AI が攻撃補助精神(魂/熱血/必中) `2589471` /
> 回復アビリティ `1aab43d` / マップ兵器 `677dcc7` を使い、ChangeMode「逃亡」`38dad3a`・「護衛」`6aa9b9a` を尊重する。
> ブランチ `feat/cc-attrs-recovery`、全テスト緑。push/マージはユーザ指示待ち。

> **2026-06-15 追記 (続き5 — BossRank サブシステム)**: 新サブシステム **BossRank** を構築（`UnitInstance.boss_rank`＋
> `BossRank [unit] rank` コマンド＋ランク別ステータス強化 HP/装甲/運動性/攻撃力）。これにより BossRank 依存の CC
> 属性を解禁: **即死(即)** `31083ec`（特殊効果発動率で非ボスを致死化、撃破サイトへ流す・即武器はクリ置換）・
> **死の宣告(告)** `52f83e5`（次の自軍フェイズで HP→1）・**石化/衰滅 のボス耐性**（石化無効・減衰半減）。テスト 5 件。
> **→ 特殊効果攻撃属性は 習(ラーニング技サブシステム要)・魅了/憑依(§3 で意図的非対応)・耐性弱点の与ダメ(要 C# 確認) を
> 除き全て実装済**。

> **2026-06-15 追記 (続き6 — 防御特性ファミリ＋切り払い)**: C# (`Unit.cs`) を精査し原典準拠で実装。**耐性/弱点の
> 発動確率**を SRC 準拠に修正 (弱点 +10 / 耐性 ÷2、`5bfe31d`)。**防御特性の与ダメージ** (耐性÷2/無効化0/吸収-1/2回復、
> 弱点有効は与ダメ変化なし＝`Unit.cs::Damage`、`08db9cc`)。**切り払い** (パイロット技能、格闘系を prob=2×防御Lv−攻撃Lv で
> 無効化＝`Unit.cs::CheckParryFeature`、`294618d`) ＋直撃の切り払い/サポートガード無効化・捨て身の反撃時無効。
> **→ 特殊効果攻撃属性・防御特性ファミリは 習(必要技能ゲート要・書式複雑で回帰リスク)・魅了/憑依(§3 意図的非対応) を
> 除き完全実装**。残る大課題 (A2/スパロボ戦記/GBA) はデータ・設計判断・複数セッション規模でユーザ依存。

> **2026-06-15 追記 (続き7 — 敵AI深掘り 4 件＋底力)**: §C 敵AI深掘りの残りを消化。**補給アビリティ**
> (`ai_use_support_ability` を回復＋補給へ拡張、`68d0cf0`) / **敵対象アビリティ** (脱=気力減少 を最も気力の
> 高い敵へ、`ai_use_offensive_ability`、`68d0cf0`) / **回復系精神** (信頼/友情/愛 で半分以下の味方を回復、行動
> 非消費の free action、`ai_use_support_spirit`、`911b70e`) / **召喚アビリティ** (劣勢時に増援、劣勢ゲートで
...
> **2026-06-15 追記 (続き8 — パイロット技能のダメージ系＋行動不能の絶対命中)**: `Unit.cs::Damage` を精査し
> 与ダメージ系のパイロット技能を実装。**潜在力開放** (高気力 130 以上で与ダメ ×1.25、`9c9df05`、predict のダメージ段、
> 静的 features 参照) / **得意技・不得手** (技能データ `得意技=格射` の各文字が武器 class に一致で ×1.2 / ×0.8、
> `a4f0513`)。さらに **行動不能の防御側を絶対命中に** (`2264e93`): 麻痺/睡眠/凍結/石化/行動不能 のとき命中 100
> (従来は麻痺/凍結のみ＝`Unit.cs`「動けなければ絶対に命中」)、**睡眠 (寝こみ) の被ダメ ×1.5** を追加 (既存の
> 麻痺/凍結 ×1.5 は温存)。これで 眠/石 等の特殊効果攻撃属性で付与した状態が攻撃側に正しく有利化する。回帰テスト
> 3 件。いずれも predict (combat.rs) 内で全戦闘サイト (通常/反撃/援護) に一貫適用。**残スキル**: ハンター
> (対象カテゴリ別ダメージ＝対象名マッチ要)・ブースト/♀武器 等の細目。test/clippy/wasm-check 全緑。

> **2026-06-15 追記 (続き9 — 気力経済の実装＝最重要)**: **戦闘で気力が上昇しない大穴を解消**。これまで気力は
> 精神/CC属性でしか動かず、魂(120)/熱血(80)/潜在力開放(130) 等の高気力技能が実戦で発動しなかった。`Unit.cs` 準拠で
> 実装: **撃墜による気力上昇** (撃破者トータル +4 / 同陣営の出撃中ユニット +1、性格「機械」は不動。`apply_kill_morale`
> を award_kill_rewards に追加＝通常/反撃/援護の全撃破サイト経由・マップ攻撃は非経由で対象外) ＋ **損傷による気力上昇**
> (攻撃を受けて生存したユニット +1、3 戦闘サイトの生存分岐に追加)。これで「戦って気力を上げ→精神/底力/潜在力開放が
> 発動」という SRC の中核ループが機能する。両変更とも既存 1830 テストを 1 件も壊さず (撃墜/被弾は結果を assert する
> テストが多く気力閾値を跨がない)。さらに **被撃破陣営の性格別変動** (超強気+2/強気+1/弱気-1/機械0) を実装し
> award_kill_rewards に victim_party を引き回し (撃墜時の気力変動を完全実装)。さらに **命中時/損傷時/失敗時/回避時
> 気力増加スキル** を実装 (`apply_combat_morale_skills` を通常/援護/反撃の命中・失敗分岐に配線、基本変動に上乗せ)。
...
> さらに **武器の自動選択が残弾/EN/必要気力を尊重するよう修正** (消費だけでは空の武器が撃ててしまう半端状態の解消)。
> `weapon_firable`＋`best_firable_weapon_in_range` を新設し、通常攻撃 (`pick_attack_weapon` に firable 述語追加・全 4 呼出)・
> 反撃・援護の自動選択を発射可能な武器に限定 (強制武器=プレイヤー明示選択は UI ゲート前提で射程/移動後のみ確認)。
> これで `is_weapon_available` 相当のゲートが実戦の武器選択に効き、**武器資源管理 (消費＋選択制限) が完成**。回帰テスト 1 件。
> **教訓 (memory にも記録)**: `execute_attack`/`best_available_weapon` のように「実行時資源を尊重する完全版メソッドが存在するが
> ライブ経路は射程・威力だけの簡易版を使う」二重実装パターンがある。同種を疑うなら未使用の完全版と app.rs ライブ経路の差分を見る。

> **2026-06-15 追記 (続き11 — A2 合体/分離の精緻化)**: 残課題サマリ A2 を消化 (`9bd0a89`)。**3機ルール**:
> 合体コマンドは 3 体以上合体のみメニュー表示 (`combine_partner_count>=2`)・2 体合体は相手ユニット上への移動で合体
> (`combine_lists_partner` を相手1機=3トークンに限定)・`combine_partners` を「列挙パートナー形態が全て 2 マス以内に揃う」
> 要求へ変更 (従来は 1 機でも在れば合体可だった大穴)。原典 合体.md「合体コマンドは3体以上のみ・2体は移動で・参加全機が
> 2マス以内」準拠。**最初から合体形態で配置されたユニットの分離**: 構成 uid を持たないユニットを `分離` 特殊能力
> (`分離=<名称> <形態1> <形態2>…`) で分離 — host を形態1 へ・形態2 以降を隣接空きマスへ新規生成・パイロットは形態順に
> 乗り移り余り(サポート)は形態1 へ集約 (変形系特殊能力.md/分離.md)。`apply_separate` を 構成温存(`separate_preserved_components`)
> /形態生成(`separate_into_forms`) の 2 経路に分岐、`pilot_name_for_id` 新設。回帰テスト 4 件 (3体合体メニュー+全相手吸収/
> 2体合体は非表示/全相手未到達で非表示/初期合体形態の分離生成)。**A2 残**: 着地点選択 (移動範囲からの UI 対話=ヘッドレス検証
> 不可・優先度低)・初期合体形態の分離後の再合体トラッキング・分離パイロットの主/副細分配分。test/clippy(wasm)/wasm-check 全緑。

VB6 製 SRC (Simulation RPG Construction) を Rust + WebAssembly に移植中。
本ドキュメントは作業継続のための要約。**解決済み課題は §9 に 1 行で要約**し、本文は
「現状・残課題・恒久リファレンス」に絞る。

**現在地（2026-06-15 セッション末）**: 直近セッションの実装ログは上の「2026-06-15 追記 (続き7〜11)」を参照。  
**テスト**: `cargo test -p src-core` 全緑（**1840 件**）／ clippy clean（`-D warnings`）／ wasm `cargo check` OK。  
**ブランチ／コミット**: feature ブランチ **`feat/cc-attrs-recovery`**（master から **67 コミット**先行、最終 feature
コミット `9bd0a89`）。**全てコミット済・master 未マージ・push 待ち**（CLAUDE.md の no-auto-push 規約によりユーザの
明示指示でマージ＋push する）。  
**本セッション（2026-06-15）で実装（19 feature + 7 docs）**:  
- **敵AI深掘り**（§C 残）: 補給アビリティ / 敵対象アビリティ(脱) / 回復系精神(信頼·友情·愛) / 召喚アビリティ。  
- **パイロット戦闘技能**: 底力·超底力（命中/回避 +30/+50）/ 超反応·超能力·底力（クリティカル）/ 潜在力開放（高気力で
  与ダメ×1.25）/ 得意技·不得手（武器属性別 ±20%）/ 行動不能の絶対命中＋睡眠の被ダメ×1.5。  
- **気力経済を完全実装**: 撃墜（撃破者+4·同陣営+1·機械不動）＋被撃破陣営の性格別変動（超強気+2/弱気-1）＋
  被弾+1 ＋ 命中時/損傷時/失敗時/回避時 気力増加スキル。→「戦って気力を上げ→精神/底力/潜在力開放が発動」する中核ループが機能。  
- **武器の資源管理を完成（配線漏れ大穴の二つ目）**: ライブ戦闘が武器の EN/弾数を一切消費せず無限だった大穴を解消
  （consume_weapon_resources を全攻撃の発射時に配線）＋武器の自動選択が残弾/EN/必要気力を尊重（weapon_firable／
  best_firable_weapon_in_range）。`is_weapon_available` 相当のゲートが実戦に効くようになった。  
> **横断的な学び（memory に詳細）**: 「実行時資源を尊重する完全版メソッド（execute_attack / best_available_weapon）が存在するのに、
> ライブ経路（app.rs）は射程・威力だけの簡易版を使う」二重実装の配線漏れが複数あった。気力・武器資源はその典型。
> なお **地形による HP/EN 回復は実装しないのが正解**（`Unit.cs` 参照実装でコメントアウト＝意図的に無効。母艦の +50% のみ有効）。  
**次セッションの残（いずれも個別ブロッカーあり）**: ハンター（対象カテゴリ別ダメージ＝技能データ書式が docs 未定義・検証
シナリオ無し）/ 見切り（分身の確率全回避システム要・RNG 列退行リスク）/ 魅了·憑依（§3 意図的非対応＝設計判断）/ 習
（必要技能ゲート＝検証データ無し）/ スパロボ戦記（84MB 実機ファイル要）/ GBA 戦闘アニメ（複数セッション規模）。
...
| A2 | 🔶 **合体/分離の精緻化**（`2c81342`/`9bd0a89`） | ✅ **合体時のパイロット統合**（全員搭乗。`pre_combine_pilots` で host の元搭乗構成を保持し、構成ユニットの `pilot_ids` を合体形態へ集約。分離で各機へ復帰。技能ボーナスが戦闘に効く）。✅ **3機ルール**（`9bd0a89`）: 合体コマンドは 3 体以上合体のみ表示（`combine_partner_count>=2`）・2 体合体は相手上への移動で合体（`combine_lists_partner` を相手1機=3トークンに限定）・`combine_partners` は全パートナーが 2 マス以内に揃う要求へ（原典 合体.md）。✅ **最初から合体形態の分離**（`9bd0a89`）: `分離=<名称> <形態1> <形態2>…` を読み host を形態1 へ・形態2 以降を隣接生成・パイロットは形態順に乗り移り余りは形態1 へ（`apply_separate` を 構成温存/形態生成 の 2 経路に分岐、`separate_into_forms`/`pilot_name_for_id` 新設）。**残**: 着地点は隣接自動（原典は移動範囲から選択＝UI 対話が要りヘッドレス検証不可・優先度低）・初期合体形態の分離後の再合体トラッキング・分離パイロットの主/副細分配分 |
...
| A4 | 🔶 **特殊効果攻撃属性の残り**（§0.5、`1d23396`/`0ab3e33`/`9fe72ec`） | ✅ **反撃/援護でも proc**（`try_counterattack`/`try_support_attack` の命中・生存分岐に `apply_weapon_special_effects` を追加。撃破/復活時は付与せず）。✅ **proc が crit を置換**（特殊効果武器は通常クリティカルしない＝`attack_resolve_and_run` で `critical && !has_special_effect`）。✅ **能力補正系属性**（`低攻`→攻撃力ＤＯＷＮ ×0.75 / `低運`→運動性ＤＯＷＮ 命中回避-15。あわせて `状態=攻撃力ＵＰ/運動性ＵＰ` 等の補正状態を combat へ反映）。✅ **命中率低下系**（`盲`→盲目・`撹`→撹乱。盲目/撹乱で攻撃側命中 ×0.5、盲目側への被命中 ×1.5。`e61ad98`）。✅ **回復阻害/沈黙/狂戦士**（`害`→回復不能 / `ゾ`→ゾンビ `5f295b8` / `黙`→沈黙 `b057b00` / `狂`→狂戦士 `118998f`）。✅ **バリア中和/踊り**（`中`→バリア半減無効 `fd3c94b` / `踊`→行動不能 `2b4c610`）。✅ **クリティカル減衰**（`衰`→HP減衰・`滅`→EN減衰、Lv1=3/4…Lv3=1/4 `b871373`）。✅ **位置移動系**（`吹`/`Ｋ`→ノックバック `cdbce74` / `引`→引き寄せ・`転`→ランダム転移 `48f35f1`、盤外/占有で停止・XL/移動力0で不発・衝突ダメージ未モデル）。✅ **気力減少**（`脱`/`Ｄ`→気力 -5×Lv（省略10）を proc 時に適用、crit も置換。Ｄ の吸収は未対応。`6c1b189`）。✅ **資金奪取**（`盗`→クリティカル時に相手修理費の1/4を資金獲得、相手1体1回。アイテム盗みは未対応。`0fcb26d`）。✅ **耐性/弱点（発動確率＋毒率）**（`adjust_proc_for_resistance`＝`耐性=`/`弱点=` と武器属性一致で発動率 ÷2／×2 `de95704`、毒ダメージも 毒弱点で倍・耐性で半減 `e450b28`）。✅ **弱/効/剋**（`弱<属性>`/`効<属性>`→`弱点:属性` condition＝後続同属性の proc 倍、`剋<属性>`→`剋:属性` condition＝該当属性武器を封じる。`06a4be7`）。✅ **移動力DOWN**（`低移`→`移動力ＤＯＷＮ`＝effective_speed 半減、`移動力ＵＰ` も +1。`0a22c6d`）。✅ **写/化**（クリティカル時に発動者を対象形態へ変化、写はサイズ制限。`faa4659`）。✅ **恐怖**（`恐`→敵 AI が味方から逃走。`dea9147`）。✅ **BossRank サブシステム＋即死/死の宣告**（`UnitInstance.boss_rank`＋`BossRank` コマンド＋ランク別ステータス強化。`即`→特殊効果発動率で非ボスを致死化・`告`→次の自軍フェイズで HP→1・石化/衰滅 もボス耐性。`31083ec`/`52f83e5`）。✅ **防御特性ファミリ（耐性/弱点/有効/吸収/無効化）の与ダメージ**（C# `Unit.cs::Damage` 準拠＝耐性÷2/無効化0/吸収-1/2回復、弱点有効は与ダメ変化なし。発動率は弱点+10/耐性÷2。`5bfe31d`/`08db9cc`）。✅ **切り払い＋直撃の切り払い無効化**（`Unit.cs::CheckParryFeature` 準拠＝格闘系を prob=2×防御Lv−攻撃Lv で無効化。`294618d`）。**残（ごく一部・実装しない理由が明確）**: 習（ラーニング技サブシステム＝技習得＋必要技能ゲートが要り、書式が複雑で回帰リスク大）・魅了/憑依（相手勢力の上書き＝§3 で SRC 準拠の意図的非対応）。→ **これ以外の特殊効果攻撃属性・防御特性は全て実装済** |
| A5 | **精神 決意/気迫/希望** | シナリオ独自（東方夢想伝）で原典定義が無く確定不能。暫定実装のまま（決意→必中+熱血 / 気迫→気力+20 / 希望→必中+集中）。実シナリオの定義が判れば `apply_spirit_effect` を修正。✅ **捨て身の反撃時無効**（`8ec9257`）・✅ **直撃のサポートガード無効化**（`40a9bde`）・✅ **切り払い（パイロット技能、`294618d`）＋直撃の切り払い無効化**は実装済。残: 捨て身の「被弾まで継続」（lifetime 延長のみ） |
...
### C. 敵 AI の深掘り（中〜大）— ✅ §3 の主要ギャップを解消

AI は機能している（`ai_runner_tick`/`ai_act_unit`）が浅かった。§3 が挙げた主要ギャップを一通り配線:
✅ **攻撃補助精神を使う**（攻撃前に 魂/熱血/必中 を発動。`ai_use_offensive_spirit`、`2589471`）。
✅ **回復アビリティを使う**（射程内の最も負傷した味方を回復。`ai_use_support_ability`、`1aab43d`）。
✅ **マップ兵器を使う**（2 体以上巻き込める照準へ発射。`ai_use_map_weapon`、`677dcc7`）。
✅ **ChangeMode「逃亡」**（敵から逃走。`38dad3a`）／✅ **「護衛」**（守護対象へ随伴。`6aa9b9a`）。
✅ **補給アビリティを使う**（EN を消費した味方を補給。`ai_use_support_ability` を回復＋補給へ拡張、`68d0cf0`）。
✅ **敵対象アビリティを使う**（脱＝気力減少 等を射程内の最も気力の高い敵へ。`ai_use_offensive_ability`、`68d0cf0`）。
✅ **回復系精神を使う**（半分以下まで負傷した味方を 信頼/友情/愛/根性 で回復。行動非消費の free action。
`ai_use_support_spirit`、`911b70e`）。
✅ **召喚アビリティを使う**（数的劣勢 味方数≤敵数 のとき増援を召喚、劣勢ゲートで乱発抑止。`ai_use_summon_ability`、`575e2c2`）。
**残（さらなる深掘り余地・balance 判断要）**: 復活精神の能動 pre-buff（撃破予測が要る）・地形/陣形の戦術判断
（防御地形選好・マップ兵器回避の散開等、大規模）。穴ではなく深掘り余地（§3 参照）。
...
- **Tier 1 残り**: 決意/気迫/希望 の効果確定（§0.1。シナリオ独自で原典定義が無く確定不能）。
  ...
  ✅ 残ニュアンス: **捨て身の反撃時無効** `8ec9257`・**直撃のサポートガード無効化** `40a9bde`・
  **切り払い (パイロット技能、Unit.cs::CheckParryFeature 準拠) ＋直撃の切り払い無効化** `294618d` を実装。
  残: 捨て身の「被弾まで継続」(lifetime 延長のみ)。
- **敵AI の深掘り**: ✅ §3 の主要ギャップ解消済（攻撃補助精神 `2589471` / 回復アビリティ `1aab43d` /
  マップ兵器 `677dcc7` / ChangeMode 逃亡 `38dad3a`・護衛 `6aa9b9a`）。残: 攻撃用/補給/召喚アビリティの能動使用・
  精神の対象選択系・戦術判断。穴ではなく深掘り余地。
