...
**テスト**: `cargo test -p src-core` 全緑（**1864 件**）／ clippy clean（`-D warnings`）／ wasm `cargo check` OK。  
...
- ✅ **必要技能ゲート（2026-06-16 実装、`feat/necessary-skill-gate`）**: `(念力Lv3)` 形式の括弧条件で武器/アビリティの
  ...
  （ステータス閾値・同調率・霊力・生身）は fail-open で誤封印回避。**残フォローアップ**: ① 撃墜数の*戦闘中加算*（規定数撃墜で
  ザコが武器解禁）は未配線＝静的読みのみ（D が上流ブロック中で未到達のため保留）。② **習（ラーニング技）**はゲート完成で
  前提解消＝実装可能に（クリティカルで相手技を pilot features へ習得→ゲートが読む）。③ 必要技能未達武器のステータス画面
  非表示（機能ゲートは完了、表示フィルタは未）。④ 格闘Lv200 等のステータス閾値は fail-open（モデル化は将来）。
...
| 必要技能 | ✅ **必要技能ゲート（2026-06-16 実装）** | `(念力Lv3)` 形式の括弧条件を `necessary_skill` モジュールで評価し武器/アビリティ可否ゲートに配線済。撃墜数Lv* 等が機能。残: 撃墜数の戦闘中加算・**習（ラーニング）の実装**（ゲート完成で前提解消）・未達武器の画面非表示（§1.1） |
