...
**テスト**: `cargo test -p src-core` 全緑（**1959 件**）／ clippy clean（`-D warnings`）／ wasm `cargo check` OK。  
...
- **差分 harness のユニット/combat 状態拡張（2026-06-17）**: `tools/oracle-diff` は式層・Commands 層（逐次）に加え、
  **✅ 静的データ層（stage a-1）を構築・実バグ 1 件を是正**。新設 `oracle_loaddata` bin（Rust）が C# `LoadDataDirectory` と
  ...
  - **✅ stage h = 防御特性「弱点」是正済（2026-06-18、11 件目）＋ oracle はこの領域を検証不能と判明**:
    ...
    - **残（要精緻化・任意・低優先）**: 援護/反撃経路の防御特性適用（`defense_attribute_damage` は通常攻撃のみ呼出。共通ケースはメイン経路でカバー済＝低インパクト）・吸収×クリティカルの適用順（VB6 は Damage 内で反転→外で ×2）。
  - **（参考・解決済の経緯）当初の調査記録**:
