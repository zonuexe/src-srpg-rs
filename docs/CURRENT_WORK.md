...
過程で **combat 実バグ 4 件**を発掘・是正（全て VB6 裏取り）: ①命中率クランプ `clamp(5,95)`→上限なし・最低0 ②最低ダメージ `max(1)`→既定10
③地形命中修正の符号（正=防御地形・`(100-hit_mod)`）④防御側パイロット Defense（耐久 `100+5*Lv`）。①②は全戦闘・③は terrain.txt シナリオ・④は耐久持ちに波及。
詳細は §1.1 stage c。  
