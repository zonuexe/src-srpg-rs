...
- ✅ **D スパロボ戦記（進行不能）＝解決済（2026-06-16・§2）**: `VERIFY_ENTRY="@2" VERIFY_AUTOSTART=1` で root から駆動すると
  ...
  すべて正しく機能していた（**エンジン修正不要**）。✅✅✅ **戦闘が走らない真因も特定済**: auto-drive のキャラメイキングが
  **味方ユニットにパイロットを乗せていない**（`pilot=""`→`effective_combat_data=None`→攻撃が静かに不発）。これは headless auto-break が
  名前入力を空で素通りするアーティファクトで、正規プレイでは起きない。次セッションの修正＝「キャラメイキングで実パイロットを生成・搭乗させる
  drive 拡張」（独自 click UI 駆動 or `召喚キャラデータ作成` ショートカット）。いずれも進行不能ではない。詳細は §2。
...
③ **これは「進行不能」ではなく、ヘッドレス auto-drive 固有のアーティファクト**: 正規プレイ（ブラウザ）はキャラメイキングの名前入力で
パイロットを作成・搭乗させるが、auto-break drive は `召喚画面`（独自 click UI／`Wait Click`＋`KeyState`）の名前入力を空で素通りするため
**パイロットが作られず機体に乗らない**。**次セッションの修正タスク = 「キャラメイキングで実パイロットを生成・搭乗させる」**:
- **(a)** drive に `召喚画面` の独自 click UI を座標＋KeyState で駆動するモードを足し、名前入力に非空値を与える（§旧 (a)）。または
- **(b)** `召喚キャラデータ作成`（CMaking.eve L746）を `召喚キャラ[名前/姓/性別/愛称/画像/…]` を既定値で set してから call するショートカット
  （drive に `VERIFY_SETVAR`/`VERIFY_CALL` 機構を追加）。
- いずれにせよ**パイロットが機体に乗れば** `effective_combat_data` が解決し、autoplay で戦闘が走る（→ そこで初めて勝敗まで通せる）。
- なお**エンジン側に小さな堅牢化余地**: 味方 combat_data=None でも攻撃を黙殺せず警告を出す／パイロット欠落ユニットを出撃前に弾く、は
  検討に値する（ただし正規プレイでは起きないので優先度低）。
