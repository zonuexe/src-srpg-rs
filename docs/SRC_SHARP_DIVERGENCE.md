...
cross-engine **10/10 一致**。捨て身/狂戦士/攻撃力ＵＰ・防御側 被ダメージ低下（鉄壁/不屈/バリア）は範囲外で従来どおり（名前ベース）＝
double-application 回避。**残（任意・低優先）**: 防御側 被ダメージ低下/被ダメージ増加 の data 駆動化、捨て身/攻撃力ＵＰ の効果統合。
