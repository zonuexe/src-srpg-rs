...
| 飛行/水中等の特殊移動コスト | 2 game MP（C# のリテラル 2 をスケール取り違え） | 1 game MP（C# `FillMoveCost` 内部 2 倍スケールの `move_cost=2`＝1 game MP） | （本セッション） |
| ブースト（ユニット特殊能力）の高気力ダメージ | 未配線（潜在力開放 のみ判定） | 高気力 130↑ で攻撃側 `ブースト` でも ×1.25（`UnitWeapon.cs` `IsFeatureAvailable("ブースト")`、潜在力開放 と独立） | （本セッション） |
| 攻撃側ダメージ増加精神の倍率 | 名前ベースのハードコード（熱血×2/魂×3/気合×1.2）・複数は積で加算 | sp.txt データ駆動 `1 + 0.1*ダメージ増加Lv`・複数は **MaxDbl 非加算**・気合=気力増加で与ダメ非増（`UnitWeapon.cs` ~3465） | （本セッション） |
| 不屈（防御側精神）のダメージ減衰 | `damage.min(1)`（1 に固定） | `被ダメージ低下Lv9`＝×0.1（`UnitWeapon.cs` down-mod。例 1572→157）。鉄壁=Lv7.5(÷4)は同値で挙動不変 | （本セッション） |
...
飛行/水中等の特殊移動コストは新設の移動範囲オラクル（`moverange`／`oracle_move`）で発見。平地では両エンジン完全一致だが、
飛行ユニットが非空地形を飛び越える際 C#=1 game MP（2 倍スケールの 2）/旧 Rust=2 game MP と乖離。C# `Map.cs::FillMoveCost`
は移動コストを内部 2 倍スケール（平地=2＝1 game MP）で扱い、特殊移動の `move_cost = 2` は 1 game MP を意味する。旧 Rust は
この **リテラル 2 を game スケールへそのまま移植**したため、飛行/水上/水中/宇宙の地形通過が全て 2 倍のコスト＝移動範囲半減していた
（pervasive）。`TRAVERSE`(=1) 定数へ集約して是正、オラクルで cross-engine 一致を確認（残るのは地上ユニットの 1.5/2.5 地形の
ceil 既知設計乖離のみ＝`CLAUDE.md`「移動範囲計算は整数に統一」）。**注**: C# の `IsAdopted`（地形適応 特殊能力）による
コスト上限 cap は Rust 未モデル（別の既知ギャップ）。

ブースト（ユニット特殊能力）の高気力ダメージは気力/精神オラクル（`combat_morale.txt`）で発見。気力 150 で C# のみ
ダメージ ×1.25 していた。C# は高気力 130↑ で 潜在力開放（パイロット技能）と **独立に** 攻撃側ユニットの `ブースト`
でも ×1.25 する（`UnitWeapon.cs` 別 if）。旧 Rust 予測は 潜在力開放 のみ判定。**これは `全ユニット共通` 継承の問題ではない**:
共通能力は unit.txt の各ユニットブロックに明示列挙され（`全ユニット共通` はタグ行）、パーサは `ブースト=…` を features へ
取り込むため、真因は予測が `atk_unit.features` の ブースト を未参照だったこと。配線で是正、オラクルで気力 150 一致（1206→1507）。
✅ **是正済（攻撃側ダメージ増加精神の data 駆動化）**: `SpecialPowerData.effects` に sp.txt の `ダメージ増加Lv` をパースし、
`db::sp_damage_increase_level`（active 精神の最大 Lv・DB 優先/既定テーブル fallback）で解決。`predict_with_status_terrain` は
`1 + 0.1*Lv`（**MaxDbl 非加算**）を適用、気合=気力増加で与ダメ非増（×1.0）。`combat_morale.txt` で熱血+魂=×2.5(210)・気合=×1.0(1020) が
cross-engine **10/10 一致**。捨て身/狂戦士/攻撃力ＵＰは範囲外で従来どおり（名前ベース）＝double-application 回避。
✅ **続いて防御側もデータ駆動化（本セッション）**: C# の完全な up/down-mod（up=`max(1,1+0.1*atk.ダメージ増加)+0.1*def.被ダメージ増加`、
down=`1-0.1*atk.ダメージ低下-0.1*def.被ダメージ低下`）へ揃え、`DamageSpiritLevels` 構造体＋`db::sp_effect_level` で 4 効果種別を解決。
ハードコードの `鉄壁→/4`・`不屈→min(1)` を撤去。**不屈 の重大乖離を是正**（×0.1。例 1572→157、旧 1）。鉄壁=Lv7.5(÷4) は同値で挙動不変。
バリア は強度吸収シールド（`DefenseMode::Barrier`）＝sp.txt の被ダメージ低下ではないため ÷2 近似のまま範囲外（二重適用なし）。
